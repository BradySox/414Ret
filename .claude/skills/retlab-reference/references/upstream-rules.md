...
> **This file describes UPSTREAM.** The RetLab fork adopted these as its own standards
> (2026-07-20), with the deltas flagged inline below as **RetLab:**. Where a delta exists, the
...
| Tech stack | Python 3.10+, Qt6 (PySide6) UI, pyDCS for mission generation, web frontend built with npm — **RetLab: pinned to Python 3.11** |
...
**RetLab:** CI black checks the **whole tree** (`.`), CI mypy only `game tests` — so a `qt_ui`
