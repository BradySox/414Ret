...
- **Open (awaiting review):**
  ...
  - [#891](https://github.com/dcs-retribution/dcs-retribution/pull/891) blue-block miz markers load + bind blue (**draft**, opened 2026-07-20) — inventory item 17 carried upstream in its final scoped+bounded shape (`prefer_blue` on the 8 marker-class callers only, `BLUE_BLOCK_MAX_DETOUR` 50 km, economy classes byte-identical, the actionable `generate_ewrs` error). The **dynamic_spawn→NEUTRAL drop stays fork-side** (it reverts an upstream feature — merge-discipline divergence, not carried). Upstream sweep: **465 dropped markers across 9 campaigns** vs the fork's 22/7 — normandy_full (352) + normandy_small (91) carry authored blue Tunguska/Scud/Silkworm marker belts the fork never had (it deleted the Normandy campaigns); `normandy_small` headless-verified end-to-end (54 bind blue / 21 by proximity), and the PR flags the Normandy resurrection as a maintainer judgment call with a pruning offer. Tests minus the fork-divergence case ported.
