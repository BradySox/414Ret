...
### Upstream PR ledger (**refreshed live 2026-07-20** — 41 PRs: 13 open / 8 merged / 20 closed. **Three fork PRs merged upstream 2026-07-19** (#805/#843/#854) and geofffranks' #859 (the §56 motorpool source) landed the same day — all four reconciled back into the fork in the `sync/upstream-dev-2026-07-19` merge. Same day, Wave 3 opened: the Splash Damage defaults PR #880 pushed (item 21, the first last-mile carve), the VWV v3.2.0 update #881 pushed (item 22), the §76 paradrop carve #884 opened (un-drafted 2026-07-20 + Starfire13 pinged for review), the **infrastructure pair #882 (Lua plugin harness) + #883 (MIST 51-symbol shim, stacked on #882)** opened as drafts late that night, and #828 was rebased — briefly un-drafted, then deliberately re-drafted the same evening (21:36→21:40Z per the PR timeline), so it sits as a draft with the un-draft call open. Still re-verify with `gh` before acting; this goes stale fast.)
