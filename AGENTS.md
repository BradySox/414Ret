...
- [docs/dev/design/](docs/dev/design/) — per-feature design notes (read before touching the
  ...
  - `414th-comint-notes.md` — **blue-side COMINT** (§70, the §51 mirror; all 5 squadron calls
    ...
    F-16/A-10 listen-only) · CW beeps v1). **C0 (the campaign take) LANDED 2026-07-18** =
    feature §70; still to build: C1 (the audible UHF red net off the comms nodes,
    `radioTransmission` + ADF hunts) and C2 (the concealed clandestine-transmitter TGO + the
    active-nets listing that briefs the hunt)
...
70. **COMINT collection (blue-side communications intelligence)** — the §51 mirror (design
    ...
    **bomb-it-or-tap-it is emergent**, never special-cased) + alive concealed COIN spawns
    ...
    preseeded — Red Tide is locked; post-M2 candidates = Red Tide + both COIN campaigns). C1
    (the audible UHF red net) + C2 (the clandestine-transmitter DF hunt) are the design note's
    next phases. Tests `tests/fourteenth/test_comint.py`; features doc §70, checklist B22 —
