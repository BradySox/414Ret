...
**⛔ UPSTREAM PR FREEZE (learned 2026-07-20):** dcs-retribution is accepting **no NEW PRs until their next beta release** (expected the weekend of 2026-07-25/26); **updating existing PRs is fine**. Holds until it lifts: the #891 consistency re-carve, the **HDS re-carve** (held draft prepared — [docs/dev/414th-hds-recarve-draft.md](docs/dev/414th-hds-recarve-draft.md); its data must ALSO be re-verified after the DM's planned mod updates + a fresh pydcs export), and the pydcs exporter-hardening PR candidate. The post-mod-update queue: re-run the export (runbook in `tools/verify_mod_export.py`) → re-align the CH/HDS extensions → refresh the HDS draft.
