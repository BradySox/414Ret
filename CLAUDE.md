...
- [docs/dev/design/](docs/dev/design/) — per-feature design notes (read before touching the
  ...
  - `414th-comint-notes.md` — **blue-side COMINT** (§70, the §51 mirror; all 5 squadron calls
    ...
    F-16/A-10 listen-only) · CW beeps v1). **C0 (the campaign take) + C1 (the audible UHF red
    net + `rednet` plugin) both LANDED 2026-07-18** = feature §70; still to build: C2 (the
    concealed clandestine-transmitter TGO + the active-nets listing that briefs the hunt)
...
70. **COMINT collection (blue-side communications intelligence)** — the §51 mirror (design
    ...
    **C1 LANDED same day — the audible UHF red net**: `plan_red_net`
    ...
    `red_comms_net` (Mission Generation → Battlefield life, default **OFF**); freqs logged at
    arm (`REDNET|:`) until C2's active-nets listing. C2 (the clandestine-transmitter hunt +
    the A↔B findability tie) is the design note's remaining phase. Tests
    `tests/fourteenth/test_comint.py` + `tests/missiongenerator/test_rednetluadata.py` +
    `tests/lua/test_rednet_runtime.py`; features doc §70, checklist B22 (in-app) + B23
    (in-game).
