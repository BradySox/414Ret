...
28. **Settings IA reorg + difficulty presets** — the settings dialog + New Game wizard are
    ...
    **Theater** page ("Forces & Budget", campaign-reseeded on select; field names unchanged), leaving
    a dedicated grouped **Mods** page (Aircraft/Asset packs/Air defense — the curated 15 of ~49
...
### Upstream PR ledger (**refreshed live 2026-07-19** during the upstream sync — 40 PRs: 12 open / 8 merged / 20 closed. **Three fork PRs merged upstream 2026-07-19** (#805/#843/#854) and geofffranks' #859 (the §56 motorpool source) landed the same day — all four reconciled back into the fork in the `sync/upstream-dev-2026-07-19` merge. Same day, Wave 3 opened: #828 rebased + un-drafted, the Splash Damage defaults PR #880 pushed (item 21, the first last-mile carve), the VWV v3.2.0 update #881 pushed (item 22), and the **MOOSE/MIST-rework give began** — the Lua harness #882 + the 51-symbol mist shim #883 (items 23/24, the MANTIS on-ramp), both drafts per the new **drafts-first rule** (every upstream PR now opens `--draft`; un-drafting is its own explicit call). Still re-verify with `gh` before acting; this goes stale fast.)

Carved out of this work, against `dcs-retribution/dcs-retribution` (all authored by `bradyccox`):

- **Open (awaiting review):**
  - [#883](https://github.com/dcs-retribution/dcs-retribution/pull/883) MIST → 51-symbol compatibility shim (**draft**, stacked on #882; inventory item 24, opened 2026-07-19) — the fork's `mist_moose_shim` extended with the 11 symbols only upstream's extra consumers call (dismounts `getGroupPoints`/`marker.remove`, EW-jammer `getPitch`/`getRoll`/`makeVec3GL`, EWRS speed conversions, and the Pretense `teleportToPoint`/`respawnGroup`/`getRandomPointInZone`/`pointInPolygon` family built on the shim's `getGroupData`/`dynAdd` core); every `mist_4_5_126` reference re-pointed (plugin.json, Pretense generator, relocate tests) and the 9.5k-line file deleted; the 51-symbol contract + new-symbol behavior pinned in the harness. On the harness branch; pytest 462 passed / mypy / Black clean. The Pretense teleport family is the piece flagged for an in-game eye (the fork never ran Pretense).
  - [#882](https://github.com/dcs-retribution/dcs-retribution/pull/882) headless Lua plugin test harness (**draft**; inventory item 23, opened 2026-07-19) — `tests/lua` (harness.py + dcs_stubs.lua) carved from the fork with the fork-only pieces generalized; upstream already pins `lupa==2.7` so zero dependency ask; first consumer = 4 Splash Damage 3 runtime tests (load + percent-normalization contract + tracked Mk-84 impact detonation + unknown-weapon ignore; power values deliberately un-pinned around #880). The credibility vehicle for the Lua give queue (§58/§49/§59 → CSAR → MANTIS).
  - [#881](https://github.com/dcs-retribution/dcs-retribution/pull/881) Vietnam War Vessels support → v3.2.0 (inventory item 22, opened 2026-07-19) — upstream's VWV support was frozen at v3.0.0: registers the 3.1.0 Sampans ×5 + Junk civilian craft and the 5 never-registered hulls (Radford / Epperson / Everett F. Larson / Solon Turman / USNS Card; ids from the installed mod's own `Database/Navy/*.lua`), adds all 11 to the `faction.py` eject list, and bumps the wizard label + 4 stale faction `requirements` versions to v3.2.0. Registration-only parity with the fork (no unit yamls/prices — the fork hasn't authored them either). **Commit 2 retires the standalone OH-6 Vietnam Asset Pack toggle** (`oh_6_vietnamassetpack` + the `pydcs_extensions/oh6_vietnamassetpack` module): the pack is maintained *inside* VWV these days, every one of its 36 `vap_*` ids was already duplicated in `vietnamwarvessels.py`, and VWV's later import was already winning the `vehicle_map` registration — the module was shadowed dead weight (the `vap_*` eject collapses to the VWV gate; upstream's Pretense imports re-pointed at the surviving VWV class names; the OH-6A helicopter mod `oh_6` untouched). On dev @ `acf02b75`; pytest/Black/mypy green. Fork reconciled same day (same eject entries + VAP-toggle retirement + its own 4 stale faction strings; the 3 Vietnam campaigns' `oh_6_vietnamassetpack` preseeds dropped — all three preseed `vietnamwarvessels: true`).
  ...
  - [#828](https://github.com/dcs-retribution/dcs-retribution/pull/828) recon fog-of-war (§3) — the flagship carve. **Rebased + un-drafted 2026-07-19**: squashed to one commit on dev @ `acf02b75`, re-validated (upstream pytest 451 passed; the new ship-movement test double gained the `game.settings` chain `known_for` reads), `MERGEABLE`, ready for review.
