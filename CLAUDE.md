...
- **Freeze: STILL ON by every observable indicator.** Newest release is still v1.5.0 (2026-04-13); `test/1.6`'s tip is still `e01208c5d` @ 2026-07-25 while `dev` merged through 08-28; **no new alpha or beta branch exists** — upstream's only new branch is `kneeboard_inflight_waypoint_fix`, a working branch for #848. Only the DM lifts it.
- **13 upstream commits sit outside our ancestry; 11 are already ours by content** (#939, #928, #931, #932, #921, #942, #924, #886 and the three campaign-data ones). Genuinely open: **#947** (`MH-60L DAP` fielded by no fork faction) and **#946** (`sweden_2020`: upstream has `UH-60L` + `KC-130J`, we have `KC-135`).
