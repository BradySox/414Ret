...
* **[Mission Generation]** **Every country on the map now has its real border, and the ones staying out of the war defend them.** Countries that are nobody's ally used to be scenery. Each nation on the map -- including the one the map is named after -- is now drawn with its actual border, taken from public-domain map data and shipped with the terrain, so no campaign authors anything. **Whose side a country is on is read from the airfields inside it**: a country whose fields you hold is yours, one where both sides have a field is contested and claimed by neither, and one already in the war is drawn as an outline only. **Whether you may cross comes from the same airfields** -- you can fly over a country you fly from, and over one both sides fly from. Everything else is closed. **A country that is closed flies a standing patrol inside its own border**, in the air from the moment the mission starts, so you can see it on radar before you cross rather than discovering the border by tripping it. Cross anyway and it calls you on the radio immediately, and again if you stay. **Keep pressing -- stay past the engage timer, drop something inside the border, or shoot at the patrol -- and it turns hostile in place on your enemy's coalition, comes for you, and the country's SAM battery wakes up.** Leave promptly and nothing happens. If both sides violate the same country it puts up a second flight, one for each intruder. **This is for players only**: an AI flight that strays is warned and never attacked, and the auto-planner does not know the borders exist, so it will still route you through one -- crossing is your call to make or avoid. Patrol aircraft are free and untracked; killing them changes nothing at the turn boundary. **Flown on Syria with five countries guarding at once**, twenty aircraft: every track was checked against its own border and 23 of the 24 never crossed out, the four-ship changed sides together and kept flying, the SAM fired, and the patrol fired back. The only aircraft that leaves its country is one that has turned hostile and is chasing you. **Nobody has flown a full campaign against it yet**, and an AI flight straying across a border is still untested. Off by default: **Mission Generator -> General -> "Neutral-faction border defense"**, which also needs the **"Neutral border defense"** plugin ticked. The patrol is a four-ship carrying whatever air-to-air fit its era allows -- against a modern jet that is numbers rather than reach, deliberately. Both the warning and engage timers, and whether the borders draw on the F10 map at all, are plugin options. Takes effect on the next generated mission.
