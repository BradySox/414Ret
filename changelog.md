...
* **[Campaigns]** Enduring Resolve gets the **2006 OEF air war**: a new multi-national **OEF Coalition 2006** faction — **Dutch F-16s (322 Squadron) and RAF Harriers (IV (AC) Squadron) on the Kandahar ramp** beside the A-10s, Marine Harriers at Bastion, **carrier Hornets (VFA-113) overhead from the Arabian Sea**, and **CENTAF Strike Eagles (391st FS), B-1s, and KC-135s from the Gulf** (two new off-map spawn points), each squadron under its own nation's flag and voice. The insurgents answer with the era's honest threat: **ZU-23 sites at every stronghold and SA-9s at the five anchors** — guns and IR SAMs only, no radar SAM anywhere, so nothing stops you at altitude and everything punishes you low. NEW game required.
* **[Campaigns]** Enduring Resolve now has **the COIN lattice**: 9 permanent population-center rings over the real named towns of the 2006 campaign — Sangin, Musa Qala, Now Zad, Kajaki, Gereshk, Marjah, Delaram, Tarin Kowt, Lashkar Gah (real positions on the DCS Afghanistan terrain). The laydown's strongholds sit ON those towns, so half the insurgency's cells and caches are inside the rings: **26 targets stay AI-legal in the open desert; 26 are under positive control** — yours to strike, priced per kill (the ROE violation weight drops 6.0 → 1.0: CDE pressure, not taboo — a careful town fight is survivable once, a carpet habit bleeds the mandate out). The ratline threads the rings on the real corridors (the 611 through Musa Qala; the Helmand highway through Lashkar Gah and Gereshk). NEW game required.
...
* **[Campaigns]** The five Vietnam-era campaigns (Khe Sanh, 1968 Yankee Station, Steel Tiger, Velvet Thunder, Red Flag 81-2) now ship **`high_digit_sams: true`** and their laydowns carry **red EWR sites**, so the period **P-37 "Bar Lock"** the HDS faction pass added actually spawns — closing the "period red IADS has no early-warning net" gap in practice, not just in the faction files. New EWR markers: Hanoi + Vinh on the Yankee Station board (Steel Tiger shares it), the NVA deep rear at Khe Sanh, Saipan for Velvet Thunder (Red Flag 81-2 already had its two). **HDS Ultimate Compilation is now on the campaign Required-Mods lists.**
