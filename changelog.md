...
* **[Campaign]** **New games plan like upstream DCS Retribution by default.** The eight 414th planner gates that used to ship enabled — overlapping BARCAP waves, SEAD-window strike timing, automatic recon add-on flights, weather-aware planning, escort jammers, adaptive procurement, route-driven fuel tanks, and the continuous campaign clock — now default to upstream's stock values. A new **Planner behavior** bar at the top of the Campaign Doctrine settings page switches all eight together ("Stock (upstream)" / "414th suite"), and each can still be tuned individually afterward. Existing saves keep their current values; only new games see the new defaults. First slice of the 2026-08-09 planner re-convergence decision — the divergence audit (`docs/dev/design/414th-autoplanner-upstream-divergence-audit.md`) is the map of what changes next.
