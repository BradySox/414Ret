...
* **[Mission Generation]** **Convoy ambushes and the naval weapons-release stagger no longer depend on a LUA plugin.** Both were runtime scripts doing something DCS's own mission triggers already do, which meant an unticked plugin silently disabled the setting you had turned on — the reason seven campaigns had to force the "Convoy ambush" plugin on behind your back. Both are now written straight into the generated mission: an ambush is a hidden trigger zone on the road that springs only when *that* convoy drives into it (a passing aircraft can no longer set it off, and DCS watches the zone continuously instead of the old 15-second poll), and each warship is handed its own timed weapons-release instead of a script deciding the order at runtime. **Nothing about how either feature plays has changed** — same ambush radius, same grace period, same "TROOPS IN CONTACT" call, same 120–900 s release window and even spread. What changes for you: the two settings are now the *only* switch, so a host whose saved defaults had the "Convoy ambush" plugin unticked will start getting ambushes; the plugin itself is gone, and the "Naval magazines & weapons release" plugin is now only needed for the cross-turn magazines. Its two weapons-release timing options have moved into the generator and are no longer editable per mission.
* **[Mission Generation]** **Cloud presets: pick the weather mod you actually have.** The old **Use Bandit's clouds** checkbox only knew one community cloud pack, and that pack has been superseded — so anyone running a newer one had a tickbox that loaded the wrong presets. It is now a **Custom cloud preset pack** dropdown (**Campaign Management → General**) offering **None (stock DCS presets)**, **Bandit's Cloud Presets**, **Weather 2.0 (Bandit)** and **ATMOS-X**: pick whichever you have installed in DCS and the mission generator can use its presets. Only one can ever be active, because the packs reuse the same preset slots for different clouds — switching packs now ejects the previous one instead of layering on top of it. A save that had Bandit's clouds ticked keeps them. Adopted from upstream [#927](https://github.com/dcs-retribution/dcs-retribution/pull/927).
...
* **[Mission Generation]** **Flights stop carrying identical loadouts — and one jet now mixes old and new stock.** Every flight of the same aircraft and task used to carry a **byte-identical** loadout, so six BARCAP flights put up six identical magazines of the newest missile the campaign date allowed — turn 1 and turn 30 alike. Now **each weapon station rolls separately** for how deep into the old stock it is reaching and steps that far down that weapon's own generational ladder, so a Hornet that wants four long-range missiles can come out with **a couple of AIM-120s and a couple of Sparrows on the same jet**, and no two flights are loaded quite alike. It is not just an air-to-air thing: the bomb ladder is generational too, so BAI and Strike jets step **JDAM → LGB → dumb bomb** (GBU-31 → GBU-24 → GBU-10 → Mk 84), and anti-ship and rocket stores have depth as well. Roughly one station in five reaches for older stock from turn 1, rising each turn as the good stuff runs out — three knobs set the opening chance, the climb and the ceiling (20% / +4% per turn / 50% by default). A substitution never leaves the weapon's family, is **never newer** than what it replaced, and never touches a targeting pod, jammer or decoy; a loadout **you** customised is left exactly as you built it; and the roll is fixed once the flight is planned, so re-generating a turn won't reshuffle it. New setting `Mix old and new stock in loadouts` (**Mission Generation → Loadouts**, default **on** — turn it off for the old identical-loadout behaviour).
