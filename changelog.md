...
* **[Mission Generation]** **Optional kill tracking for strike targets that are real map buildings** (**414th Features → Strike accounting**, off by default). Strike targets come in two kinds: buildings Retribution spawns, and real terrain buildings marked out in the campaign file. The first kind is tracked by name and never misses. The second is tracked by a DCS trigger the sim checks against the map, and that check does not always fire — the object may have moved in a DCS map update, it may have no destructible body, and ED broke scenery death events outright in 2.9.7. When it misses, you flatten a building and the campaign never hears: it is standing again next turn. Turning this on puts a hidden marker object on each such target and records the kill when the marker dies, using the same name-matching that already works for spawned buildings. The two signals are combined, so nothing that works today stops working. **It is off by default because of the trade-off it makes:** the marker can be killed by a near miss, which would credit a building you did not actually destroy — a false positive in place of today's false negative. The marker type is the one a shipped commercial campaign uses for the same job. Turn it on if scenery targets not registering is a problem you have hit; report what you see. Takes effect on the next generated mission.
