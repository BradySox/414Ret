...
* **[Mission Generation]** **Your squadrons run down their good stock as the war drags on.** Every flight of the same aircraft and task used to carry an **identical** loadout, so six BARCAP flights put up six identical magazines of the newest missile the campaign date allowed — forever, turn 1 and turn 30 alike. With the new `Squadrons run down their good stock as the campaign wears on` setting (**Mission Generation → Loadouts**, default **off**), each flight instead rolls for how deep into the old stock it is reaching and steps that far down the weapon's own generational ladder: AIM-120C to AIM-120B, and on a deep roll **all the way to the Sparrows**. The chance climbs every turn — three knobs set where it starts, how fast it rises, and the ceiling it stops at (0% / +4% per turn / 50% by default) — so the opening turns are well supplied and the late campaign is flown on what is left in the bunker, with the occasional flight going two or three generations back. Substitutions never leave the weapon's family and **never touch a targeting pod, jammer or decoy** (several of those declare a missile as their emergency fallback, which is fine when the campaign date forces it and absurd here). Every jet in a flight carries the same thing, a loadout **you** customised is left exactly as you built it, and the roll is fixed once the flight is planned — re-generating the turn won't reshuffle it.
