...
* **[Campaign]** **New games plan like upstream DCS Retribution by default.** The seven 414th planner gates that used to ship enabled — overlapping BARCAP waves, SEAD-window strike timing, automatic recon add-on flights, weather-aware planning, escort jammers, adaptive procurement, and the continuous campaign clock — now default to upstream's stock values. A new **Planner behavior** bar at the top of the Campaign Doctrine settings page switches all seven together ("Stock (upstream)" / "414th suite"), and each can still be tuned individually afterward. Existing saves keep their current values; only new games see the new defaults. First slice of the 2026-08-09 planner re-convergence decision — the divergence audit (`docs/dev/design/414th-autoplanner-upstream-divergence-audit.md`) is the map of what changes next.
...
## Fixes
* **[UI]** **The New Game window no longer opens with its buttons off the bottom of the screen.** The app fits a window to your display when it opens, and the New Game wizard is at its smallest at that moment — the first page is a short one. Clicking Next to reach the campaign list nearly doubles its height, and nothing re-checked the fit, so the window grew downward from where it had been placed and the Back/Next/Cancel row ended up under the taskbar. On a 1440p panel at 150% scaling it hung 36 pixels over the edge, and stayed there for the rest of the wizard — including the settings pages, which you then could not leave without moving the window by hand. The wizard now re-checks its fit whenever it changes size, so it slides back onto the screen as it grows. It is moved the shortest distance that works and never shrunk, so a window you dragged somewhere stays roughly where you put it, and a wizard that already fits is not touched.
* **[Campaign]** **The front line was being drawn onto ground the armour cannot drive on.** Where the front sits along the supply route is worked out first; how wide it runs is worked out afterwards, by measuring outward from that point until it leaves ground vehicles can cross. When the front landed on the *edge* of that ground -- a mountain pass, a shoreline, the lip of a valley -- the measurement inverted: the side with the usable ground stopped at zero, and the side with none of it ran the full distance because there was no edge out there to stop at. **Caucasus - Northern Russia, turn 1, Kutaisi against Khashuri FOB: the whole 20 km front lay on the ridge, with none of it on the drivable ground** -- which was all on the other side, from the crossing out to 6 km. The mission then could not place anything on the line it had drawn, so it fell back to dropping each side on the nearest patch that worked: blue bunched on one edge of the ridge, red on the other, **15 km apart with a mountain between them** and no battle at all. The width is now measured against the ground that is actually drivable. In open country nothing changes and no campaign's front moves. Where the front is pinched into a pass it is **legitimately narrower than the width setting asks for** -- that same front now runs 6.15 km, all of it on ground both sides can fight over. A flank backed against terrain does **not** hand its unused width to the other side: that would widen fronts everywhere there is terrain on one side and pull the fight away from the supply route the two sides are contesting. Takes effect on the next generated mission; no new game needed.
* **[Campaigns]** **Caucasus - Northern Russia: the road from Kutaisi to Khashuri went over the mountains instead of through them.** The campaign's supply line reached the Likhi range in one **41 km straight leg**, and one of its own waypoints sat on ground vehicles cannot cross. Every other route in that campaign has 15 to 39 waypoints and follows roads; this one had eight. Because the front line is placed by distance along that line, the fight kept landing on the ridge rather than in the pass. It now follows the E60/S1 -- the Rioni valley through Terjola and Zestafoni, then the Chkherimela valley and the Rikoti pass down to Surami. The longest leg drops to 11.2 km, every waypoint is on drivable ground, and 98% of the drawn line is (was 75%). Sampling 15 positions along the route, the front now sits on drivable ground at all 15 instead of 10, and averages slightly *wider* than before. NEW game required -- routes are read when the campaign is loaded.
* **[Campaign]** **A campaign that overrides one of its own supply routes no longer links the two bases twice.** Routes come from the campaign mission file, and a campaign YAML may override one. The override replaced the route correctly but added a second connection between the same pair of bases, and the ground planner counts those connections rather than ignoring repeats. No shipped campaign hit this before now, because none had overridden a route its mission file already defined.
