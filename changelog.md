...
* **[Training]** **New — Formation Lead Trainer.** The DCS AI wingman holds position no matter how you fly, so leading a flight with it tells you nothing about whether a person could stay with you; the first real feedback a lead gets is usually a human overshooting into him. This puts a **virtual wingman** on your wing that is deliberately not glued there — he reacts late, his throttle, drag and G run out, and he falls out of position where a human would. While you fly he calls the roll-ins and turns he cannot stay with, naming what he is short of: *"outside man needs +58 kt he has not got"*, *"needs 2.7g of throttle"*. F10 gives a scored debrief — the percentage of the flight he could hold position, and every fault as a count of separate mistakes. **The limits move with the spacing you pick** (fingertip, route, 0.5 nm cruise, 1.5 nm combat spread), because a wingman on the outside of a turn must fly faster by the turn rate times how far out he is, and must acquire that speed just as fast. Same turn, twice the spacing, twice the demand. At 1.5 nm **no bank angle is followable at all** — which is the reason tactical turns exist, and the tool derives it rather than asserting it. **A training mission ships with it**: four client Hornet slots airborne over the Caucasus, a six-exercise syllabus in the briefing, and the script embedded so it needs nothing installed — fly it solo or fill the slots with the squadron. The script also drops into any mission as a DO SCRIPT FILE, and runs on a Retribution turn as a plugin, **off by default**. It is read-only: it spawns nothing, commands nothing and cannot affect your aircraft or the mission. **Stated plainly: the numbers are reasoned, not measured.** The model is self-consistent and tested, but nobody has yet flown a real two-ship against it to check that its wingman falls out where a human does. Expect the envelope to need tightening or loosening after the first pass.
