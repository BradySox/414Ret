...
* **[Mission Generation]** **A carrier launches in waves.** The last fix stopped overflow aircraft vanishing by counting every jet in the mission against the deck's 16 spots, so after the first 16 everything started airborne however long the deck had been empty — on one Syria turn, 15 of 53 left the deck. The deck is now counted as it empties: a jet holds its spot until five minutes after its planned takeoff, a package parks or starts airborne as a whole, and no more than four Tomcats are parked at once — the pile-up that jammed an earlier carrier deck. The same turn now launches 23 from the deck in seven waves. Player flights still always keep their spot. Takes effect on the next generated mission.
...
* **[Mission Generation]** **A carrier no longer loses the aircraft that will not fit on its deck.** The auto-planner frags every squadron the boat holds, and nothing checked how many would fit. On one Syria turn it put **50 aircraft in 24 groups onto CVN-71**: DCS parked 14 and then silently refused the rest — **33 aircraft, 14 whole flights, gone with no error, no crash and no event**, including both of blue's SAM-killing packages. Two flights launched; the other twelve sat hot on deck for the full 71 minutes. The deck now fills to **16 parking spots**, the number the DCS Supercarrier Operations Guide gives for mission start, and anything beyond that **starts airborne** instead of waiting on a hangar deck that never clears. **Player flights are never moved** — a human keeps the deck spot the briefing promised, and the AI behind them takes the air start. **The trade, stated plainly:** the guide also says an F-14 blocks the spots beside it, so a Tomcat-heavy deck can still fill before 16 and lose the last jet or two to the hangar; modelling that needs footprint data DCS does not publish. Nothing to configure. Takes effect on the next generated mission.
