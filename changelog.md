...
* **[Mission Generation]** **A SAM's escort no longer sits where you last found it.** When a point defense wakes to intercept a HARM, it gives away its position — and then stayed on that exact spot for the rest of the campaign, so one anti-radiation shot bought you a permanent map pin. The MOOSE engine has always been able to displace those groups, but it needs somewhere to send them, and nothing in DCS creates those destinations; a hand-built mission has to place them in the editor one at a time. Retribution now generates them: each point-defense group gets a handful of hidden destinations on driveable ground around it, and after the group has been woken or fired on it relocates up to about 1.3 NM. A Tor or Tunguska that defended a site once is somewhere else by the time you come back for it. **Off by default** — it puts ground units on the move, which is the one thing that cannot be checked outside a live mission, so it is opt-in until it has been flown. Find it under **MANTIS IADS → Point-defense shoot-and-scoot**; it needs the SEAD-triggered point defense, which is on. NEW mission generation required — the destinations are written when the mission is built, so turning the option on against a mission you already generated does nothing.
