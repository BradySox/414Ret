...
### B7 — Red Intent: adaptive enemy posture · §55 · ☐ UNTESTED (built 2026-07-08; the posture classifier, asymmetric hysteresis, all four seam helpers, the byte-identical no-ops when off/blue/stock, the offensive-order reorder, the stacked unpredictability + §52 C2 clamp, the aggressiveness bias, and the stance-factor + `red_tempo` yield are unit-tested in `tests/fourteenth/test_red_intent.py` + `tests/test_planner_unpredictability.py`; whether red's *played* behaviour visibly changes with posture across a multi-turn campaign needs a fly)
- **What CI cannot exercise:** whether a RED side actually plays *differently* as its posture shifts — surging (pressing captures/CAS, committing ground reserves at a lower advantage, focused targeting) when it holds the advantage vs consolidating (defending, husbanding, slightly erratic targeting) when it's been hit — and whether the posture transitions read sensibly turn-over-turn given the memory + hysteresis.
- **Setup:** enable **Red Intent** (Air Doctrine) on a campaign where red has real offensive agency (a peer fight — Red Tide is a good case). Play several turns; deliberately swing the balance (win/lose ground, bomb/spare red's forces) while watching the "Enemy posture" SITREP line and red's ATO.
- **Pass:** the SITREP names a posture that tracks the war (surge when red is ahead, consolidate after red loses a base / is pushed back) and it doesn't flip every turn (hysteresis holds); when SURGE, red visibly prioritises taking ground and commits to aggressive stances sooner; when CONSOLIDATE, red defends/husbands and its target picks are a little less repetitive; **reactive defense is unchanged** throughout (§17 boundary); an active authored `red_tempo` pulse (Vietnam arcs) still owns the stances (P3 yields). Turning the setting off restores stock red exactly.
- **Fail signature:** red plays identically regardless of posture (a seam isn't reaching the planner — check `_offensive_order`'s red branch, `effective_aggressiveness`, `stance_commit_factor`); the posture flips every turn (hysteresis broke); consolidate forces red into RETREAT rather than DEFENSIVE (the attack-stance-only gate broke); the posture ignores last turn (the baseline/`last_sitrep` memory isn't wired); any change with the setting off (the gate broke); the stance bias fights an authored `red_tempo` pulse (the yield broke).

### B8 — Strikeable motorpool depots: strike the reserve, force a repurchase · §56 · ☐ UNTESTED (adopted from upstream PR dcs-retribution#859, 2026-07-08; the reserve split (`reserve_armor_for` == `plan_groundwar`), the populator cap/round-robin/idempotence, the generator's parked/weapon-hold/no-datalink render + offset depot, the 1:1 `base.armor` decrement, and the motorpool-vs-front-line loss separation are unit-tested across `tests/**/test_motorpool_*.py` + `tests/ground_forces/test_reserve_armor.py` — but no fork campaign authors a `Fortification.Garage_A` yet, so nothing renders in-game until one does)
- **What CI cannot exercise:** whether an authored depot actually renders (the maintenance-facility icon on the map; the in-mission `Garage_A` building + the grid of parked reserve vehicles), whether killing a parked vehicle decrements the owner's `base.armor` and forces a repurchase next turn *without* shifting the front line, whether the spawn cap holds, and whether the debrief shows the motorpool losses.
- **Setup:** a campaign that places a `Fortification.Garage_A` static at a CP (none ship yet — author one in the ME, or add a `motorpools` preset location). Play with `motorpool_enabled` on; note the depot, strike some parked vehicles, end the turn.
...
### G9 — Combat SAR AI on-demand rescue (`auto_combat_sar`) · §21 · ☐ UNTESTED (REWORKED 2026-07-06 — the standing orbit is retired; `auto_combat_sar` now spawns an on-demand rescue **parked-first / clone-fallback** when a pilot goes down and no player CSAR package is up. The gate + parked/clone emit are unit-tested (`tests/missiongenerator/test_combat_sar_sandy_luadata.py` + `test_combat_sar_templates.py`); whether the rescue helo actually launches, flies the OPSTRANSPORT pickup, and delivers needs a fly. The old orbit's 2026-06-28 "good" verdict below is now moot — NEW model, NEW game required)
