...
### G9 — Combat SAR AI on-demand rescue (`auto_combat_sar`) · §21 · ☐ UNTESTED (REWORKED 2026-07-06 — the standing orbit is retired; `auto_combat_sar` now SPAWN-clones a cold rescue-helo template on demand when a pilot goes down and no player CSAR package is up. The gate + template + emit are unit-tested (`tests/missiongenerator/test_combat_sar_sandy_luadata.py` + `test_combat_sar_templates.py`); whether the cloned helo actually flies the OPSTRANSPORT pickup and delivers needs a fly. The old orbit's 2026-06-28 "good" verdict below is now moot — NEW model, NEW game required)
- **What changed (2026-07-06 on-demand rework, §21):** the auto-fragged orbit (`PlanCombatSar`) is deleted; when a pilot goes down with no player CSAR/SCAR package fragged, the combatsar runtime clones the `spawn_combat_sar_templates` cold helo template into the pickup (the proven clone path, not the retired commandeer). **Pass:** with `auto_combat_sar` ON and NO CSAR package fragged, eject near the front → `dcs.log` shows the clone (`CombatSAR Rescue N#001`) launching + an OPSTRANSPORT to a friendly field + a delivered survivor spared at debrief; with a player CSAR package fragged, NO auto clone spawns (autoSpawn=false). **Fail:** the clone never launches / never reaches the survivor / RTBs without picking up (the old orbit failure mode); or a clone spawns despite a fragged player package (the gate broke). The historical orbit-era notes below are retained for context only.
