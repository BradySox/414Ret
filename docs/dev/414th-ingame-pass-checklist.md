...
| 2 | ~~**§72 carrier deck decor — position drift + a floating static**~~ | B25 | ✅ **CLOSED 2026-08-06 by non-reproduction** — WATCH item 4, DM verdict "Passing": gear on the deck, nothing floating, nothing out of place. No code changed; the diagnosis in the row stands as the record. **Caveat kept:** one session on an unrecorded hull/variant (6 rotate), and the ~10 m aft / ~5 m outboard `CORRAL_SHIFT` drift from raw OCN is **accepted, not fixed** — do not re-seat on raw OCN without re-validating `KNOWN_PARKING_SPOTS`. | Answered: it looks right now. If a float ever resurfaces, note WHICH hull + WHICH static. |
