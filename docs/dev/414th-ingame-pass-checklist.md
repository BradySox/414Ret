...
### B31 — Graduated escort jamming (ESCORT_JAMMER tiers + growler plugin) · §77 · ☐ UNTESTED (built 2026-07-21, reworked same day from Growler-only to graduated tiers; the planner role, the tier roster + effect gradient, the loose-tier gate, the emitter tier knobs, and the whole Lua policy — grace, tier-scaled spoof bubble, FULL-only WEAPON_HOLD pulse + OPEN_FIRE restore, radar-SAM-only eligibility, friendly-fire guard, defensive-only tiers never pulse, player-off + F10 menu — are pinned in tests/fourteenth/test_escort_jammer.py + tests/missiongenerator/test_growlerluadata.py + tests/lua/test_growler_runtime.py. PASS: a curated vanilla jammer (e.g. an AI F/A-18C or F-16C, mods off) rides a strike package into a radar-SAM ring and a radar missile aimed at a package member disappears mid-flight without an explosion at the launcher (defensive bubble), and a dedicated jammer (Growler/Prowler, mod on) ALSO makes the SAM visibly check fire in pulses (offensive, FULL tier only); a player jammer sees the F10 "Growler jamming" menu and jamming stays dark until toggled ON; with escort_jamming_loose OFF the AI only fields curated jammers, ON it will frag a podded stand-in (e.g. an A-10) at token strength. FAIL signatures: an ECM/self-protect/loose jammer pulsing a SAM (offensive gate broken), a SAM never firing at all (hold stuck — restore path broken), the jammer orbiting away from the package (escort plan regression), a friendly missile vanishing (guard broken), MANTIS alarm-state weirdness near the pulsed site, or a loose jet auto-fragged with the setting off (gate broken). **Balance (2026-07-21, off the "2-4 ship × up to ~12 jammers" note):** effects don't stack — a missile faces the single strongest bubble (rolled once, deterministically pinned that a 2nd identical jammer doesn't raise the spoof rate), a suppressed SAM gets a mandatory recoverySec shoot-back window before any jammer can re-hold it (pinned via the hold→release→hold cycle), and a per-side max_escort_jammers cap (default 4, 0=off) counts ATO jammers in can_plan_escort. PASS additions: fly a mass of jammers (loose on) into a dense IADS and confirm the SAMs still fire in windows (not permanently dead) and every missile isn't spoofed; confirm no more than max_escort_jammers jammer flights are fragged in a turn. FAIL additions: SAMs permanently silent under many jammers (recovery window broken), near-100% missile spoof under overlapping bubbles (stacking not fixed), or more than the cap fragged)
...
- **Deliberately not in scope — HOLD on the new airfields (DM call 2026-07-26):** do not bind a CP to any 2.9.28 airfield yet. The airfields **are usable**; the blocker is that **the southern part of the Iraq map is not detailed**, so Kharg (and Zaranj on Afghanistan, Tromso on Kola) wait for ED to finish that extension. Kharg additionally sits 565 km from the nearest airfield of either Iraq campaign and needs the pydcs pin bump before a CP could bind at all. Also out of scope: rail interdiction (§35 is road-graph-only; new engine work).
