...
### B24 — Expanded F-4E Weapons Pack: HARM Weasel fits · §71 · ☐ UNTESTED (built 2026-07-18; the (XW) selection, the Shrike fallback, the editor hiding, and the 1988 era-gate tripwire are unit-tested in `tests/fourteenth/test_f4e_expanded_weapons.py` and the Red Tide preseed is pinned — whether DCS itself mounts and employs the injected AGM-88s on the Heatblur F-4E is mod/DCS-only)
- **What CI cannot exercise:** the modded jet in DCS — whether the installed mod build actually accepts the AGM-88C on stations 1/3/11/13 as generated (a mod update can rename/renumber stores), whether an AI Phantom employs it against emitters, and the mod-off failure mode (stores stripped at spawn).
- **Setup:** Red Tide NEW game with the Mods-page "Expanded F-4E Weapons Pack" box left checked (preseeded) and DSplayer's mod installed on the server + clients. Plan an F-4E-45MC flight on SEAD (air-wing-dialog squadron or host frag); confirm the Payload tab defaults to "Retribution SEAD (XW)" with 4 HARMs; fly or spectate it against an emitting red SAM.
- **Pass:** the Phantom spawns carrying 4 AGM-88Cs (rearm screen shows them mounted), SEAD/SEAD Escort/SEAD Sweep all default to their (XW) fits, an AI-flown flight launches HARMs at emitters, and §54 scarcity debits the same `arm` family the Hornets draw from. On an otherwise-identical game with the box UNCHECKED: the same flight defaults to "Retribution SEAD" (Shrikes) and the payload editor lists no (XW) fits.
- **Fail signature:** Phantom spawns with EMPTY ARM stations (DCS stripped the stores — mod missing on the server, or a mod update changed the pylon acceptance; re-verify against the installed mod's lua, the §41 HDS drill); an (XW) fit selected in a mod-off game (the pylon gate broke — check `Loadout.pylons_allow` and that `eject_F4E` ran via `apply_mod_settings`); AI SEAD Phantoms holding fire inside range (distinct from simply not being tasked — the F-4E's SEAD task priority is a deliberate 120, so it flies SEAD on frag/overflow, not first pick).
