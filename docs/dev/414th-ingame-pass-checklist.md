...
- **Residual (minor, doesn't hold the verdict):** SHORAD intercept unobserved — a Tor 9A331 sat at the target but never engaged (likely already dead from the player's DEAD strike; missiles arrived unopposed), so "MANTIS/SHORAD engages the inbounds" is still unseen in anger; the `#N` marker-text salvo sizing untested (default salvo 4 used); the CH `*_LACM` Kalibr hulls + a red-side raid unflown; full magazine exhaustion → "ship goes silent" untested.
