...
- **Setup:** a preseeded campaign (COIN Enduring/Inherent Resolve, 1968 Yankee Station, or Red Tide — `convoy_ambush` + the `convoyambush` plugin are ON; **NEW game**). Advance a turn; on the F10 map find the friendly convoy moving between two blue bases (it looks like any other convoy — there is deliberately NO ambush marker and NO escort package in the ATO). Fly anything, and decide on the TIC call whether to divert. May take a few turns to catch a hit roll (~50 % per convoy; `dcs.log` "CONVOYAMBUSH|: armed N ambush(es)" > 0 confirms a live one without spoiling positions).
