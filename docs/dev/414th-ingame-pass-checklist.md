...
### B10 — Mission-start briefing popup: the slot-in card · §58 · ☐ UNTESTED (built 2026-07-11; the emitter shape (shared header + one record per player-crewed flight, AI-only excluded, gated off) is unit-tested in `tests/missiongenerator/test_briefingluadata.py`, and the runtime (birth shows exactly one card with every field + the right group id/duration, the mission-start sweep catches a seated player, an AI birth / unknown group / absent node show nothing, the birth+sweep debounce) is harness-tested in `tests/lua/test_briefing_runtime.py`. The harness models no DCS UI, so whether the text actually renders on screen is DCS-only.)
- **What CI cannot exercise:** whether `trigger.action.outTextForGroup` actually paints the card on the pilot's screen, that it reads correctly (campaign / Mission N / date / time / callsign / aircraft / task / field), that it clears after its duration, and — the two paths — that it fires both at mission start in single-player (the sweep) and on a mid-mission slot-in / rejoin on a server (the birth handler), each exactly once.
- **Setup:** leave **"Mission-start briefing popup"** on (default). Generate and fly any mission with a player flight; watch the screen the instant you take the slot. For the server path, join a running mission and slot in mid-flight.
- **Pass:** a card appears on slot-in for ~12 s showing the campaign name, `Mission N`, the date + mission time, and your flight's callsign / aircraft / task / departure field; it shows once (no double-print), and a re-slot after the debounce re-shows it.
- **Fail signature:** no card appears (the node wasn't emitted, or the birth handler + sweep both missed the slotting); the card double-prints on a single slot-in (the debounce broke); an AI-only flight's pilot slot shows a card for a flight that isn't theirs (the group match broke); the wrong field/date/time (the header sourcing drifted from the kneeboard).
