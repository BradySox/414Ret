...
- **Setup:** **Germany — Red Tide** now **preseeds `red_intent` ON** (2026-07-08) — and `war_economy` too, so the supply→posture loop is live — so a NEW Red Tide game exercises this directly. Play several turns; deliberately swing the balance (win/lose ground, bomb/spare red's forces, and **bomb red's supply**) while watching the **"ENEMY &lt;posture&gt;" chip on the map ribbon** + the "Enemy posture" SITREP line + red's ATO.
