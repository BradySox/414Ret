...
### A7 — QRA react-task filter (AI QRA ignores sweeps/BARCAP/DEAD/Air Assault) · §1 · ☑ VERIFIED (2026-08-05, user pass `units-runway-generation-bf755e` — "A7 good") (was ◐ PARTIAL, 2026-07-17 night fly: behavior consistent with the filter — red QRA sat through 20+ min of blue fighter presence and scrambled only once A/G packages pushed; not conclusive on intent)
...
### B9 — Air-droppable minefields: mine a road, kill a convoy, carry the field across the turn · §57 · ⊘ RETIRED (⛔ SHELVED 2026-07-30 — dropped from active use by user call, not deleted; every gate defaults OFF and Red Tide's preseed was removed, so nothing runs in any current campaign. Not a pending test while shelved; code/tests untouched (`tests/lua/test_minefields_runtime.py`, `tests/fourteenth/test_minefields.py`, `tests/missiongenerator/test_minefieldluadata.py`). Re-open as ☐ UNTESTED if the feature is ever resumed.)
...
- **CJS Super Hornet bullet (added 2026-08-02):** slot a client **F/A-18E/F or EA-18G** and confirm the cartridge AutoLoads the same way — COMM presets named to the kneeboard, steerpoints + route sequence with ETAs, and (carrier flight) the §65 TACAN/ICLS/ACLS pre-tuned. **Expected absence, not a fault:** the SA page carries **no** FLOT, no CAP/tanker racetracks and no MEZ rings — the mod's DTC descriptor has no `SA` table, so those sections are deliberately never emitted. Fail signature: the cartridge is listed but un-loaded (the mod's descriptor didn't accept our JSON — diff `<mod>/DTC/FA-18F_DTC.lua`'s `data` table against what `superhornet.py` emits, since the mod can change it between releases); a garbled COMM/WYPT section (the mod stopped delegating to ED's `CoreMods/aircraft/FA-18C/DTC` implementations — the byte-identical-to-Hornet test would still pass, so this is DCS-only); no cartridge at all on the DTC page (the airframe id isn't in `CARTRIDGE_BUILDERS`, or the mod build in use ships no descriptor for that variant — the ET/FT tanker variants are excluded on purpose).
...
### G39 — Point-defense shoot-and-scoot: the woken PD displaces afterwards · §89 · ☐ UNTESTED (built 2026-08-10, closing the bridge's long-standing "AddScootZones needs Python-generated zones" deferral. The generator side — which groups get zones (mirroring `collect_pd`'s SAM/SamAsEwr-only scan and its shared-PD dedupe), the ring lying inside MOOSE's `minscootdist`/`maxscootdist` selection window, the land check, name uniqueness, the radius clamps at both ends, seeded determinism, and every off-path — is unit-tested in `tests/missiongenerator/test_iads_scoot_zones.py` (19). What no test can reach: whether the group actually drives, and where it ends up.)
- **What it is:** `shoradScoot` (plugin option, **default OFF**, requires `shoradLink`). The mission generator rings each point-defense group with hidden `RetributionScoot-N` trigger zones on driveable land; the bridge hands them to `MANTIS:AddScootZones`. MOOSE then displaces a PD group up to ~1.3 NM whenever it wakes for a HARM/Maverick shot, is shot at, or stands back down.
- **What CI cannot exercise:** whether the group physically drives (the harness models no DCS AI or pathing), whether it arrives somewhere sane rather than into a building/cliff/water edge the landmap called land, whether it re-engages after arriving, whether the displacement breaks the site's own formation or leaves the guarded SAM naked, and the frame cost of N moving groups on a SAM-dense campaign.
- **Setup:** any MANTIS campaign with PD-escorted SAMs. **NEW mission generation required** — with the option off at generation time the miz has no zones, and the bridge logs `point-defense scoot is ON but the mission contains no 'RetributionScoot' zones`. Enable **MANTIS IADS → Point-defense shoot-and-scoot**, regenerate, and check `dcs.log` for `point-defense scoot armed: N displacement zone(s), M-M m legs`. Red Tide is the same testbed as G30 (SA-15 Tor + SA-19 Tunguska). Fire a HARM at a PD-escorted site from inside ~10 NM, then watch that site in Tacview for the next few minutes. ~30 min.
- **Pass:** the arm line shows a plausible zone count and a sane leg range; after the wake the PD group drives a few hundred metres to a couple of kilometres and stops on land; it is still combat-effective where it lands; the guarded SAM is still covered; no framerate change.
- **Fail signature:** `scoot armed: 0 displacement zone(s)` on a PD-rich campaign (the generator's node scan drifted from `collect_pd`, or the prefix stopped matching — same class as the G6 zero-resolve and G30 zero-arm bugs); the PD never moves despite the arm line (the distance window inverted, or the zones landed outside `minscootdist`..`maxscootdist` — the one contract two files have to agree on); a PD group driving into water or off a cliff (the landmap said land where DCS disagrees); a group that drives and then never engages again; PD wandering far enough to abandon the SAM it exists to protect, which is the feature defeating its own purpose.
...
### H12 — Back-to-upstream kneeboard deck (upstream pages + folded 414th info) · §31 / §30 · ☑ VERIFIED (2026-08-05, user pass `units-runway-generation-bf755e` — "H12 is good") (was ☐ UNTESTED, reworked 2026-07-13; the 2026-07-05 back-to-basics render pass covered the now-retired cover/Brief-Sheet deck)
...
### Q1 — Per-aircraft flight defaults save + apply · §43 · ☑ VERIFIED (2026-08-05, user app pass `pr-merge-code-audit-7e8b4c` — "Q1 and Q2 are good") (was ☐ UNTESTED, built 2026-07-02; store/apply fully unit-tested, the button + the "new flight opens pre-configured" behaviour is Qt UI)
...
### S7 — Measured fuel data adopted from DCS Liberation drives tanker + bingo for 12 airframes · §46 · ☐ UNTESTED (adopted 2026-08-07 from the DCS Liberation research pass — twelve `fuel:` blocks copied verbatim for A-10A/A-10C/A-10C_2/A-4E-C/AV8BNA/F-100D/the four F-14 marks/F-15C/F-4E-45MC, fanning out to 16 variants; measured coverage 22 → 40 aircraft types. All 16 pinned in `tests/dcs/test_estimated_fuel_consumption.py`)
...
### B42 — Old-stock loadout attrition · §84 · ⊘ RETIRED (**FEATURE REMOVED 2026-08-06** — flown, disliked, and ripped the same day. WATCH item 1 put eyes on it for the first time and the DM's verdict was *"I've seen and disliked, revert or rework"*, resolved to a **full rip**; the specific objection was **turn 1 already downgraded**.
...
### B43 — SAM battery support section (refuellers + power) · §85 · ☑ VERIFIED (2026-08-06, WATCH item 3, DM verdict "Passing" — the support section renders on SAM sites in a post-2026-08-04 game; no bare-launcher-and-a-jeep site and no bowser-instead-of-truck displacement) (was ☐ UNTESTED, built 2026-08-04)
...
### B44 — Support kit at legacy SAM sites + fuel convoys · §85 · ☑ VERIFIED (2026-08-06, WATCH item 3, DM verdict "Passing" — legacy sites field trucks **and** a bowser, i.e. the slot-displacement bug this wiring was built to fix did not reproduce) (was ☐ UNTESTED, built 2026-08-04)
...
### B47 — Missile battery support section + priced launchers · §85 · ☑ VERIFIED (2026-08-06, WATCH item 3, DM verdict "Passing" — missile batteries render the transport/transload/fuel/command section rather than three launchers and a jeep. Verified the **same day it was built**, so the composition was seen on a genuinely fresh game) (was ☐ UNTESTED, built 2026-08-06)
...
> Everything below the runtime is unit-tested (`tests/missiongenerator/test_carrier_deck_decor.py`,
> 17): the forward-deck envelope, clearance against every known spot, the landing-area
> exclusion, gating and rotation, and the invariant that a recovery placement must never reach
> the `.miz`. What CI cannot exercise is whether MOOSE `SPAWNSTATIC:InitLinkToUnit` actually
> produces a static that rides a steaming deck.

**Enable** `carrier_deck_decorations_recovery` (Mission Generation → Carrier; needs the main
decorations toggle on) on a Nimitz-hull campaign, then fly or watch one recovery.

- **Pass:** nothing stands on the bow at mission start. When the launch set is struck below,
  gear and (on some variants) parked jets appear forward on the starboard bow, 3-9 pieces depending on the rotating variant and **ride the deck** — still in place,
  still square to the ship, after the boat has steamed a few miles.
- **Fail, three distinct signatures worth telling apart:**
  1. **Nothing appears.** MOOSE absent or `SPAWNSTATIC` errored — check `DECKDECOR|` in
     `dcs.log`; the spawn is `pcall`-wrapped and logs a count, so a 0 is diagnostic.
  2. **Gear appears, then the boat sails out from under it.** The link failed and the statics
     are world-anchored. That kills the tier as designed.
  3. **A jet spawns into the new gear.** The bow spots (11/12/13) are unmeasured — this is the
     known risk and the reason the tier is default-OFF.
- **The measurement this row is really for:** if it flies clean, Tacview the deck at t=0 with a
