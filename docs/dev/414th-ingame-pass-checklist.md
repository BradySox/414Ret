...
- **Pass:** the campaign appears and starts; Mosul is the one red airfield with the 5 red Nineveh town FOBs (Hammam al-Alil / Bartella / Bashiqa / Hamdaniya / Tal Afar); the southern front sits partway up the Q-West axis and moves under pressure; the COIN feed (caches / IEDs / VBIEDs / emirs / dispersed cells) fires as on Enduring Resolve; SEAD has a job (the SA-6 at Mosul) while SA-8/9/13 + ZU-23 punish the deck; a fixed strike inside the Mosul box costs mandate.
- **Fail signature:** campaign hidden from New Game (version gate) or errors on load; a FOB/airfield in water or off-map; the front never forms (Q-West ↔ Hammam route missing) or captures a base by sweep; the crust never fills (faction SAM presets dropped) so SEAD has nothing; the will meters/phases don't move; the ISIS spawns read as US armor (fiction-kit retype not applied).
