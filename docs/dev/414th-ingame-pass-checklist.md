...
> **Blocked on a campaign, 2026-08-19.** The fix has landed but cannot be flown on the test
> standard: it needs an anti-ship *engagement*, and **no Starfire campaign has a red navy** —
> all fifteen carrying a Nimitz hull are blue-navy-only. Closing it means either a campaign with
> an opposing fleet or hand-adding red hulls to a `.miz`. Deliberately not done for one row.
