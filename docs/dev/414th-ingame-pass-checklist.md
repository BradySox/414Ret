...
### C11 — Front-less AEW&C stations forward · §8-adjacent (support orbits) · ☐ UNTESTED (built 2026-07-17 from the flown Scenic Route Merged A-50 — 424 NM from the enemy fleet, orbiting its own rearmost base. The #613 no-front fix correctly stopped the runaway depth march (the orbit holds AT its anchor), which exposed the OTHER half: `theaterstate.aewc_targets` picked `farthest_friendly_control_point()` — the rear-safe choice that only makes sense when a front exists for the orbit geometry to work against. On a front-less theater the non-carrier AEW&C target is now `closest_friendly_control_point()` — the friendly field nearest the enemy. Fronted campaigns byte-identical; carrier targets untouched. Unit-tested in `tests/test_aewc_targets.py`; where the orbit actually ends up is DCS-only)
- **Setup:** a front-less campaign with a land-based AWACS on either side (Scenic Route Merged red A-50 is the reference case). NEW game or a replan (aewc targets are re-derived each plan pass).
- **Pass:** the A-50/E-3 orbit anchors near the friendly field closest to the enemy (Scenic: the Bandar Abbas/Havadarya area, ~100-150 NM from the fleet), standing off the threat ring per the #613 clearance floor — close enough that its radar actually feeds the GCI picture (MANTIS `CheckLoop` climbing while it's airborne).
- **Fail signature:** the AWACS still orbiting a deep-rear field 300+ NM out (the target pick not taking effect — check `front_lines()` actually yields nothing on this laydown), or an AWACS parked INSIDE a threat ring (the clearance floor failing — a different bug, in `support_orbit_anchor`).
