...
|---|---|---|---|---|
| 1 | **Strip the packaged-drone JTAC** — "G26, 27 need stripped from the build, leave G32 as its default behavior". Removes `coin_packaged_jtac_drone` + `auto_jtac_drone`, `game/fourteenth/jtac_drone.py`, the `_maybe_configure_jtac` drone path, the mutual-exclusion gate, both COIN campaign preseeds, and the two test files. Stock front-line FAC (G32) becomes the only JTAC model. | G26 · G27 · G32 | Ordered, not started. **13 files**: `game/coalition.py`, `game/fourteenth/jtac_drone.py`, `game/missiongenerator/aircraft/aircraftgenerator.py`, `game/missiongenerator/flotgenerator.py`, `game/settings/settings.py`, `resources/campaigns/coin_enduring_resolve.yaml`, `resources/campaigns/iraq_inherent_resolve.yaml`, `tests/fourteenth/test_jtac_drone.py`, `tests/missiongenerator/test_drone_jtac.py`, + `CLAUDE.md`/`AGENTS.md`/`414th-features.md`/this file | Full rip (settings gone, §20/§40/§53–55 precedent) or leave the settings defaulted off? A full rip is the reading of "stripped from the build". |
| 2 | **§72 carrier deck decor — position drift + a floating static.** DM: "wrong place, floating in one case, just shifted out of place really from the original miz's we investigated." Two separate defects; see the row. | B25 | Diagnosed, not fixed | Which hull + which static floats? The decor rotates 6 street variants per (carrier, turn) seed, so a float may be variant-specific. |
| 3 | **G2 TARS BDA bridge needs reworking.** Passed 2026-06-24; a lot has changed through that path since. | G2 | Flagged, no detail | WHICH of the three capture paths is wrong — player F10 film, AI recon overflight, or attack-on-tasking reveal? They fail differently. |
| 4 | **I2 civilian air traffic — reverse the decision, go to RAT.** DM: "I think RAT is the real answer." The row's own title records RAT being retired in favour of Python/pydcs; this reverses that. | I2 | Design call made, unscoped | What does the Python version do badly? And is a MOOSE runtime dependency acceptable where the current path is generation-time only? |
...
### G2 — TARS BDA bridge · §12 · ✗ REGRESSED (2026-08-05, user call `units-runway-generation-bf755e` — "G2 needs reworking". Flagged pending detail: the row passed on 2026-06-24 and a lot has moved through this path since — the `airecon` AI-capture channel (2026-07-01), the drone-always-films rule and the role-aware TARPS TOT (2026-07-05), the armed-recon package changes, and the §3 concealment snap-to-exact on discovery — any of which could have changed what the bridge banks or when. **Do not start reworking until the DM says WHICH behaviour is wrong**, because "the TARS bridge" now spans three different capture paths (player F10 film, AI recon overflight, attack-on-tasking reveal) and they fail differently) (was ☑ VERIFIED 2026-06-24)
...
### I2 — Civilian background air traffic (Python/pydcs, RAT retired) · ✗ REGRESSED (2026-08-05, DM design call `units-runway-generation-bf755e` — "I2 needs a rework I think RAT is the real answer". This **reverses the original decision**: the row's own title records that RAT was retired in favour of a Python/pydcs implementation, and the DM now judges MOOSE's `Functional.RAT` (Random Air Traffic) the better answer after living with the result. Not a defect report — a direction change, so it needs scoping before any code moves: what specifically the Python version does badly (too few aircraft? wrong routes? no airport-to-airport realism? no despawn?), whether RAT is acceptable given it is a MOOSE runtime dependency where the current path is generation-time only, and what happens to the existing setting. Worth noting the fork already carries MOOSE as the standard in-mission framework, so RAT is available — the retirement was a design preference, not a technical block) (was ☑ VERIFIED 2026-06-26, user in-game pass)
...
**The three tuning knobs are on a DIFFERENT page** — Mission Generation → Loadouts — so don't
hunt for them next to the gate. Defaults are **20 %** at turn 1, +4 %/turn, 50 % ceiling, so
turn 1 already mixes (roughly one station in five); the earlier "0 % at turn 1, turn 1 is a
no-op" text here was the first cut's default and was stale. Raising `stock_attrition_start`
to ~40 % just makes the effect obvious in one look.
