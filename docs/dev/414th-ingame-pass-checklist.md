...
- **Fail signature:** no drone squadron appears (gate: `has_jtac`/`jtac_unit` a drone/TARPS-capable, setting on — check the faction JSON); a **second** drone squadron on a campaign that already fields one (the existing-drone skip broke); the drone based at the front line or an inoperable field (rear/`can_operate` pick broke); a red drone squadron auto-fielded (blue-only gate broke); the drone sits idle and never frags (no A/G packages that turn, or the auto-recon hook off — `auto_add_tarps_recon`).
