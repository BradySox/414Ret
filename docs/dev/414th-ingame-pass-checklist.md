...
- **Pass:** tow tractors / P-25 / forklift / deck hands visible along the island street and the 4 LSO figures on the port-aft platform, all riding the deck as the boat steams (no gear left behind in the water, none sliding); every cold-start flight spawns at a real parking spot exactly as without the feature (no "flight delayed" that a decoration-off regen doesn't also show); AI recoveries taxi and park normally; next turn the street arrangement differs (variant rotation). **Aircraft tier** (flip `carrier_deck_decorations_aircraft` ON for this half): the two folded SH-60s appear starboard-aft, riding the deck like the gear; cold spawns shift away from those aft spots gracefully (fewer spots is the DOCUMENTED cost — the fail is only a spawn INSIDE/clipping a static aircraft, or the six-pack/port-quarter/helo spots being affected at all). **The launch-phase E-2C (dynamic respot):** the Hawkeye stands on the stern round-down through the launch cycle, then VANISHES — silently, no explosion, no wreck — either ~35 min in (fallback) or the moment any friendly fixed-wing runs in low astern (fly a practice CASE I: by the time you're on the initial up the wake at 800 ft the deck must already be clean; `dcs.log` shows `DECKDECOR|: <boat>: struck N launch-phase static(s) below (<why>)` + the "deck respotted for recovery" text). **Fail signatures:** the E-2 still standing when you cross the ramp (cone + fallback both failed — check the log for `DECKDECOR|: armed`; absent = emit/plugin gate); the E-2 exploding or leaving a wreck on destroy (would need a different removal path); the despawn yanking it mid-view while the boat maneuvers (cosmetic, note severity); the cue text spamming (once-only latch broke).
