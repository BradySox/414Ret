...
- **Pass:** the briefing card appears on slot-in for ~12 s (campaign name, `Mission N` matching the kneeboard's turn number, date + time, your callsign / aircraft / task / departure field); ~12 s later the taxi card flashes with your callsign + `Contact ground @ 249.50`; each shows once (no double-print), and a re-slot after the debounce re-shows them.
