...
- **Pass:** each hub shows 3 dispersed single-radar S-300 fire units + a shared EWR, all one networked IADS (MANTIS); killing one battalion's radar drops that battalion but the regiment keeps engaging; the belt takes several sorties to roll back (not one HARM / one turn); every new battalion sits on open ground; front MERAD screen still shows its §60 two-radar sites.
...
- **Setup:** any MANTIS campaign with PD-escorted SAMs (Red Tide: the S-300 regiments + MERAD belt all carry Tor/Shilka escorts). Check dcs.log for `SHORAD link armed: N point-defense group(s)`. Ingress on a site with an RWR: confirm no Tor emission while inbound. Fire a HARM at the site from inside ~10 NM; watch the PD.
