...
| BC-H | Retribution | **Save as Campaign** (Increment D) on a built blank canvas → it appears in the New Game list → starting it rebuilds ownership + the SAM/EWR/armor/factory/ammo laydown | Saved campaign hidden from list (version gate); load crash; rebuilt map barren or wrong sides | ☐ UNTESTED — core (serialize + `Campaign.load_theater` rebuild) **headless-verified 2026-06-27** (ownership + preset-native counts match through real `from_file`→`load_theater`→generate); the Qt **Save** button (D.3) isn't built yet, so no in-app flow to fly until it lands |
