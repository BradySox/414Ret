...
| 4 | **I2 civilian air traffic — rebuild, but STAY IN PYTHON.** Scoped 2026-08-05 with the DM. **RAT is NOT the answer after all** and stays retired: pressed on what was actually wrong, the DM named *civil identity* and *airways vs milk runs* as "the real issues" — RAT fixes NEITHER (it wouldn't make an An-26 read as an airliner), while reintroducing the `woCharacterHuman` heliport crash that killed the rotary layer and the respawn churn that was its second crash path. Its one differentiator, ATC / living airfield, is explicitly not what is missing. | I2 | Scoped, not started | None — ready to build. |
...
### I2 — Civilian background air traffic (Python/pydcs, RAT stays retired) · ✗ REGRESSED (2026-08-05, DM rework `units-runway-generation-bf755e`. **Opened as "I think RAT is the real answer" and closed the other way in the same conversation** — worth recording, because the first read was wrong twice over. (a) **RAT was never retired on taste**: `game/missiongenerator/civiliantraffic.py`'s own header records that it was cut because it CRASHED the sim — recurring `woCharacterHuman` / GermanyCW-FARP crashes traced to RAT resolving an unresolvable heliport id and spawning a malformed unit, which is why the rotary layer had to be disabled entirely; respawn churn was a second crash path. (b) **RAT does not fix what is actually wrong**: pressed on the symptom, the DM named the two real issues as **civil identity** and **airways vs milk runs** — RAT addresses neither (it would not make an An-26 read as an airliner), and its one genuine differentiator, ATC / living airfield, was explicitly NOT among the complaints. So RAT stays retired and the rebuild stays in Python.
**The two real issues, in the DM's words:**
**(1) Civil identity is a data table.** Liveries + airline callsigns + cruise levels do nearly all of the perceptual work. The current roster leans Soviet-military — An-26 / IL-76 / Mi-8 read *wrong* over Germany or the Gulf — so **region-appropriate selection matters more than adding types**.
**(2) Airways, not milk runs.** The multi-leg meander (`FW_LEGS = 5`, `build_chain`) exists ONLY to keep an aircraft airborne long enough to cover the mission without a respawn loop. Replace it with simple high-altitude transits: cheaper, more realistic, terrain-safe.
**Likely subsumes the third complaint.** "Too little / it disappears" was also reported, and the obvious fix looked like a concurrency scheduler — but low-level milk runs pottering between rear neutral fields are *invisible* to a player at altitude, so the felt sparsity may be a VISIBILITY problem that airway transits at cruise levels fix on their own. Build (1) and (2) first and re-judge density afterwards rather than adding a scheduler up front. Also still true and cheap if density really is short: `neutral_fields()` admits only airfields Retribution does NOT control, which collapses the pool on maps where the engine owns everything — widening it to rear fields regardless of owner (front keep-out kept) plus off-map entry/exit points is the lever) (was ☑ VERIFIED 2026-06-26, user in-game pass)
