...
- **⚠ N1 IS NOW AUTHORED AT GENERATION, NOT SCRIPTED (2026-08-05 — re-read before flying).** The release stagger left the plugin entirely: each ship generates `ReturnFire` plus a `ControlledTask(OptROE(WeaponFree))` with a `start_after_time`, computed in Python (`naval_release_schedule`). **There are no `NAVALMAGAZINES|: … released weapons-free` log lines any more** and no `releaseMinS`/`releaseMaxS` plugin options — verify the schedule instead by opening the generated miz in the ME (each ship group's waypoint-0 advanced actions show the conditioned ROE task and its time). The plugin now logs only its metering arm line, and **is not needed at all unless N2 metering is on**. This shares the same DCS-only unknown as S3: whether DCS honours a **start condition on an ROE *option*** — **fail signature: every ship sits on `ReturnFire` for the whole mission and never opens up**, in which case the fallback is to author the release as a `TriggerOnce(TimeAfter(T))` trigger rule per group rather than a waypoint task.
- **Fail signature:** every group releasing in the same second (the even spread regressed in `naval_release_schedule`); ships that **never** open fire at all (see the start-condition unknown above); **a winchester or pre-release ship sitting passive while aircraft attack it** (the load-bearing unknown resolved badly — the honest options are to accept it or to abandon N1's hold, since per-weapon ROE does not exist); magazines never depleting despite launches (the fired weapon's `typeName` doesn't match `ASHM_WEAPON_PATTERNS` — read the real name out of `dcs.log`/Tacview and extend the pattern list, but **never** with a land-attack family); a magazine that debits *twice* per shot or moves at generation time (the §63 double-count or the regeneration-safety rule broke); or §63 cruise-missile raids suddenly costing anti-ship stock (a land-attack family leaked into the pattern list).
