...
| 33 | **Native DTC cartridge support in pydcs** — the per-unit `DTC` block + the `DTC/*.dtc` files. pydcs drops the block on re-save (units re-serialize from parsed fields only) and cannot author a cartridge, so §74 carries two shims in `game/missiongenerator/dtc/cartridge.py`: a `FlyingUnit.dict` wrap and a post-`Mission.save` zip append. **This is a pydcs PR, not a Retribution one** — [dcs-retribution/pydcs#34](https://github.com/dcs-retribution/pydcs/pull/34); the dcs-retribution PR freeze does not bind it, and it is an existing PR either way | 🔵 IN REVIEW — open non-draft since 2026-07-19. 2026-08-22: base merged (2.9.28), Starfire13's F-14B(U) correction applied (only `F-14BU` ships a DTC descriptor, not `F-14B`), CI green on 3.10/3.11/3.12 | Medium (when it lands and the pin moves, `cartridge.py` shrinks to the model + builders) | n/a (pydcs-side `tests/test_dtc.py`; §74 behavior rides the §74 rows) |
