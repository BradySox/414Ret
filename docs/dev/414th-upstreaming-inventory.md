...
| 22 | **Vietnam War Vessels support → v3.2.0** (upstream's VWV support was frozen at v3.0.0: registers the v3.1.0 civilian craft — the 5 Sampan variants + the Junk — and the 5 hulls the mod carries that were never registered (USS Radford DD-446, USS Epperson DD-719, USS Everett F. Larson DD-830, AD-30 Solon Turman, USNS Card T-AKV-40; ids read from the installed mod's own `Database/Navy/*.lua`), adds all 11 to the `faction.py` eject list so a faction citing them degrades cleanly with the mod off, and refreshes the stale labels — wizard checkbox v3.0.0 → v3.2.0 + the 4 faction `requirements` entries that had drifted to v0.9.0/v2.3.0/v3.0.0. Registration-only parity with the fork: deliberately NO unit yamls/prices for the new hulls (the fork hasn't authored them either — the civilian craft's gameplay wiring is the fork-only `civiliantraffic.py`). **Commit 2 retires the standalone OH-6 Vietnam Asset Pack toggle**: the pack is maintained inside VWV (the install ships it as `[VWV] Vietnam Assets Pack`; its README describes migrating off the standalone), all 36 of the `oh6_vietnamassetpack` module's `vap_*` ids were duplicated in `vietnamwarvessels.py`, and VWV's later `__init__` import was already winning the `vehicle_map` registration — so the module + `oh_6_vietnamassetpack` ModSettings field + wizard checkbox go, the `vap_*` eject collapses to the VWV gate, and upstream's Pretense VAP imports re-point at the surviving VWV class names (mypy caught them). The OH-6A helicopter mod (`oh_6`) is untouched — only its companion pack moved into VWV) | 🔵 IN REVIEW — **pushed as [PR #881](https://github.com/dcs-retribution/dcs-retribution/pull/881)** (2 commits), opened 2026-07-19; validated on dev @ `acf02b75` (pytest 438 passed / Black / mypy clean). Fork side reconciled same day: the 11 eject entries + the VAP-toggle retirement (3 campaign preseeds dropped; all three preseed `vietnamwarvessels: true`) + the fork's own 4 stale faction strings (usa_1970 v0.9.0, vietnam_1965 v3.0.0, vietnam_1970 + USA 1971 v2.3.0) | Medium/High (every Vietnam-era campaign gets the current mod roster; the stale labels sent players hunting v0.9.0–v3.0.0 downloads) | n/a (registration + version labels; upstream suite green) |
| 23 | **Headless Lua plugin test harness** (`tests/lua` — harness.py + dcs_stubs.lua carved from the fork, fork-only config setter generalized; upstream already pins `lupa==2.7` so zero dependency ask; starter suite = 4 Splash Damage 3 runtime tests against upstream's own plugin: load-without-errors + the sd3-config percent-normalization contract + a tracked Mk-84's impact detonation + the unknown-weapon ignore path — power values deliberately un-pinned around #880. Upstream's ~20 plugins had ZERO runtime coverage; this is the credibility vehicle the whole Lua give queue rides on, and the prerequisite for item 24) | 🔵 IN REVIEW — **pushed as draft [PR #882](https://github.com/dcs-retribution/dcs-retribution/pull/882)**, opened 2026-07-19 (the first PR under the drafts-first rule); pytest 442 / mypy / Black clean on dev @ `acf02b75` | Very high (turns "script dies silently, feature never starts" into a CI failure for every upstream plugin) | n/a (test infra) |
| 24 | **MIST → 51-symbol compatibility shim** (the MOOSE/MIST-rework give, step 2: the fork's `mist_moose_shim.lua` extended to upstream's full consumer surface — the audit found 51 distinct `mist.*` symbols across base/CTLD/EWRS/ewrj/dismounts/Pretense/Skynet vs the fork's 44, so 11 new implementations replicated verbatim from the MIST source: dismounts `getGroupPoints` + `marker.remove`, ewrj `getPitch`/`getRoll`/`makeVec3GL`, EWRS `mpsToKmph`/`mpsToKnots`, and the Pretense `teleportToPoint`/`respawnGroup`/`getRandomPointInZone`/`pointInPolygon` family on the shim's `getGroupData`/`dynAdd` core. Every `mist_4_5_126` reference re-pointed — plugin.json, the Pretense Lua generator, the relocate tests + test-mission script — and the ~9.5k-line vendored file deleted; rollback is the documented one-liner. The 51-symbol contract test + new-symbol behavior tests + the ported DB-tier tests ride item 23's harness. NOT given: the fork's EWRS/ewrj/dismounts/Pretense retirements (fork-context decisions upstream shouldn't inherit — which is exactly why the shim is a superset) and the Skynet→MANTIS swap (the roadmap's MANTIS-last, champion-gated item; when proposed it goes as an *alternative engine* via the dual-engine `iads_engine` selector resurrected from fork git history, default staying Skynet)) | 🔵 IN REVIEW — **pushed as draft [PR #883](https://github.com/dcs-retribution/dcs-retribution/pull/883)**, opened 2026-07-19, stacked on #882; pytest 462 / mypy / Black clean. Flagged for upstream: the Pretense teleport family deserves an in-game eye (the fork never ran Pretense — those 11 symbols are harness-validated verbatim replications, not fork-flown code) | Very high (a ~9.5k-line frozen vendored lib out of every generated miz; the fork's sync-time "grep upstream Lua for `mist.`" tax dissolves once upstream runs the contract test) | n/a (harness-pinned; Pretense fly flagged in the PR) |
...
5. Open against `dcs-retribution/dcs-retribution` **as a draft** (`gh pr create
   --draft` — standing rule 2026-07-19: every upstream give starts as a draft;
   un-drafting is its own explicit decision, never a side effect of a refresh);
   record the PR number back in the Queue table here and flip readiness to 🔵 DONE.
