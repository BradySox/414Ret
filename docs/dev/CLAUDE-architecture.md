...
`scar_command_post_intel`, now default ON for new campaigns; §15). Every accessor takes
`viewer: Optional[Player] = None` defaulting to truth; consumers gate at the edge.
...
short-circuits those three fog leaves to ground truth for any viewer, un-fogging the whole
map + intel dialogs with no server-model changes. It lives as a checkbox in `MapLayersControl`
(§19, "Enemy intel" group), driven by a `useEffect` that `PUT`s `/fog-of-war/reveal` then
re-pulls `/game`. State is never saved; resets off on every load.
...
18. **Fog-of-war overview toggle** — transient "Reveal fog of war" checkbox in the map layers
    panel (§19, "Enemy intel" group) that short-circuits all three recon-fog leaves
    (`alive_for`, `known_for`, `hidden_on_player_map`) to ground truth. Un-fogs the whole
    map + intel dialogs + hidden command posts with no server-model changes. Never persisted;
    resets off on load (`game/theater/fogofwar.py`; features doc §3).
19. **Unified map layers panel** — custom dark-themed Leaflet control
    (`client/src/components/maplayers/MapLayersControl.tsx`) replacing both stock layer controls:
    collapsible grouped sections, preset views (Default / SEAD / Recon / Clean),
    `localStorage`-persisted choices (except the transient fog reveal), and all side-effect
    toggles driven by `useEffect` (not Leaflet add/remove). Client-only; needs the CI client
    rebuild (features doc §18 / §19).
