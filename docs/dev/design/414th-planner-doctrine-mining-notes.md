...
**An empty population is a question, not a finding.** Row 6 asked whether recovery tankers get an unreachable TOT. There were no recovery tankers, in any campaign, ever. That looked like the finding and it was not — see the next lesson, which cost a closed PR.
...
| 8 | **The reachability floor on the RECOVERY branch** | ⚪ Latent, unmeasurable, leave it. The RECOVERY branch of `schedule_missions` assigns a carrier ETA with no `max(earliest_tot, ...)` — the only branch with no floor. Measured across 10 packages forced into existence: 0 unreachable takeoffs, 0 station transits over the ten-minute lead. It cannot fire while the branch itself never runs. |
