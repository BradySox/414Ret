...
**Migrate the save before measuring it.** `persistency.load_game` runs no migration — only the app's load path (`QLiberationWindow`) constructs `Migrator`. A save measured without it is a state no player ever loads. That is not cosmetic: row 6's instrument reported zero recovery tankers on a save that has three, and the difference was one `Migrator(game, False)` call. Rows 2 and 3 were measured unmigrated; neither counts RECOVERY packages, so the direction of both stands, but row 2's spread interval is sized by a package count that migration can change by a few.

**When the row cannot be measured, that is usually the finding.** Row 6 asked whether recovery tankers get an unreachable TOT. There were no recovery tankers, in any campaign, ever. Do not widen the save set until you have asked why the population is empty.
...
| 6 | **Recovery tankers are the one branch with no floor** | ✅ Built — [#978](https://github.com/BradySox/414Ret/pull/978), but **not for the reason the row said**. The row could not be measured at all: **no RECOVERY package has ever been planned in any campaign.** A squadron's auto-assignable set is `secondary | {primary}` from the campaign air-wing config and no campaign lists `Recovery`, so the carrier tanker squadron was capable, sat on the boat while dozens of aircraft recovered, and was never offered the tasking — the third instance of the hole TARPS/Escort Jammer and CSAR were each patched for. Measured: tanker squadron on the boat in 3 of 3 carrier saves, task set `['Refueling']` every time, **0** RECOVERY packages; after the fix, 3 on CVN-71 and 1 on CVN-75. Flown pass owed (B98). |
| 7 | **The missing reachability floor itself** | ⚪ **Conditionally dead, with named falsifiers.** Measured once row 6's fix made packages exist: 0/10 takeoff times unreachable and 0/10 station transits over the ten-minute lead (5–7 min). But the floor is 30 s on every measured tanker because they all started warm, runway or in-flight, and every one launched from the boat. **Re-open only on**: a COLD-start carrier campaign (surfaces the floor), or a land-based recovery tanker (surfaces the lead). Sample was 10, exactly the pre-registered minimum. |
