...
Status: **scoping only, nothing built.** Written 2026-08-20, out of the Grimes repository
assessment (`414th-grimes-repos-assessment-notes.md` §4, owed-work item 3). Every claim about
...
**External — Grimes' `script_iads_dev.lua`** (read-only; unlicensed, so ideas and DCS facts
...
2. **`S_EVENT_SHOT` fires for SAM launches** and the weapon's `getDesc().missileCategory` is
   `2` for a SAM (verified in the Grimes runtime dump: the SA-11's 9M38M1 reports
...
3. **DCS natively rearms launchers from nearby logistics.** Grimes' `warehouseDef` names the
...
4. **The clamp must be ROE, never emissions.** Hard constraint (the `enableEmission` crash);
   Grimes' script independently reaches the same conclusion (zero `enableEmission` calls).
...
- **The tubes table is the launcher filter.** Radars, C2, §85 trucks and power stations are
  ...
  the Grimes dump for vanilla — sourced facts, not copied tables.
