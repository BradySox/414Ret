# RetLab Baltic Fury Campaign Notes
...
>
> **Remaining:** CI lock `tests/retlab/test_baltic_fury.py` → sync README/feature docs → the
...
sea**: a US carrier strike group (CVN-75 Truman) in the German Bight + RetLab's USAF F-22 +
...
- **Blue faction:** `NATO Baltic 2027` (`resources/factions/nato_baltic_2027.json`) — the
  ...
  **Mod-gated:** the campaign preseeds `swedishmilitaryassetspack` + `ukmilitaryassetspack`
  ...
  `tests/retlab/test_baltic_fury.py`, along with a silent-drop guard that asserts every
...
`test_no_preset_marker_sits_on_a_runway` in `tests/retlab/test_baltic_fury.py` (no preset
...
- Quality bar / patterns: [`retlab-red-tide-campaign-notes.md`](retlab-red-tide-campaign-notes.md),
  [`retlab-sam-site-realism-notes.md`](retlab-sam-site-realism-notes.md)
