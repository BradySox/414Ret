...
[`retlab-retribution-long-view.md`](retlab-retribution-long-view.md) — none of its audits are
...
(`retlab-falcon-bms-campaign-notes.md` §3 has the full crosswalk):
...
- **Nodes** — control points holding **stocks**: ground units (`Base.armor` and friends),
  ...
  (`retlab-sam-magazines-notes.md`) are all stocks today, each with private bookkeeping.
...
- **Posture as an output, not a dial.** Red's stance emerges from its own substrate state
  ...
  (`game/retlab/red_tempo.py`) stay as the campaign designer's override on top.
...
|---|---|---|---|---|
| **R0** | Inventory: map every campaign quantity to (readers, writers, cockpit path); the substrate spec falls out | — | none (a doc) | **DONE 2026-08-20 — and the falsifier partially fired**: the core is already coherent (`retlab-substrate-inventory-notes.md`); R1 shrinks to magazine consolidation + five named couplings |
...
| **R3** | Munitions as substrate stocks: un-park SAM magazines onto R1; ground-munitions class if R2 held | `retlab-sam-magazines-notes.md`, §81 pattern | DM call | scarcity never reaches a cockpit (rule 2 breach) |
...
R0 is done — `retlab-substrate-inventory-notes.md` (2026-08-20), verdict in its §4. When any rung lands, this note gains that
