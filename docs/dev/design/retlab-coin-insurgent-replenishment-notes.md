# RetLab — COIN insurgent replenishment — design notes

**Status: C1+C2+C3 LANDED (2026-07-02); C1.5 designed (build slot after tuning); C4 design-only.** The regen core of §3 is built
(`game/retlab/coin.py`, the `coin_insurgency` setting, the `finish_turn` hook,
`tests/retlab/test_coin.py` — including the multi-turn shell-sanity test that is
...
`retlab-will-generalization-notes.md` §5/§6 (deleted 2026-08-20, recoverable from git before `5db34150f`) — squadron call 2026-07-02: COIN is the
...
One new module, `game/retlab/coin.py`, one entry point run from
...
- **C1 — the regen core** ✅ **LANDED 2026-07-02**: `game/retlab/coin.py`
  ...
  hook, `tests/retlab/test_coin.py` (anchored cap, cache throttle + floor,
...
- **C1.5 — the re-infiltration design pass** ✅ **WRITTEN 2026-07-02** (squadron call
  ...
  `retlab-coin-reinfiltration-notes.md` — a staged, announced, counterable pipeline
