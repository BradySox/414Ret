# RetLab — COIN re-infiltration (the insurgency retakes ground) — design notes

**Status: LANDED (2026-07-03).** Built in `game/retlab/coin.py`
...
one attempt theater-wide, neutral+lost scope). Tests: `tests/retlab/test_coin_reinfiltration.py`
...
`retlab-coin-insurgent-replenishment-notes.md` §7.3 (squadron call: re-infiltration
...
Extends `game/retlab/coin.py` (~150 lines): an `advance_reinfiltration(game,
