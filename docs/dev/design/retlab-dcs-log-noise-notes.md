...
| Line | Owner | Status |
|---|---|---|
| `EVENT00000.onEvent((WARNING: Could not get EVENTMETA data for event ID=61 ...))` | vendored `Moose.lua` | **Re-fixed 2026-09-13.** The 2026-08-29 fix keyed the wrong event (49, task complete); 61 is DCS 2.9.29's controller-option-change event, raised ~10/s by TIC's ROE cycling. Now guarded in `EVENT:onEvent` as upstream develop does. Before: 6,807 occurrences in the reference session, 11,861 in an archived Germany Cold War log, ~10,300 in 20 min on 2026-09-13. **Verified 2026-09-15 (test 32): 0 in 46 wall-minutes with the guard in the flown `.miz`.** See `retlab-framework-consolidation-notes.md`, *Local patches to the vendored `Moose.lua`*. |
| `INFO Scripting (Main): event:type=group change option,...` (note the lower-case `Scripting`) | DCS, the multiplayer host's event logger | **DCS's, not ours.** Every controller option change is echoed as an event line when hosting. 66,000 of test 33's 80,000 lines (19 MB, 93 min) were this, driven by TIC's ROE cycling — the same churn that fed the MOOSE event-61 spam, now in the sim's own log. Not a script error; nothing to guard. |
| `ERROR SCRIPTING: MIST|getGroupData|4148: TIC:unit|... not found in MIST database` | §9 TIC, via MIST | **Open, low.** 51 lines in one second at mission start on test 33, one per TIC unit: TIC asks MIST for group data before MIST's database has seen the groups TIC just spawned. The fight ran normally. |
| `OnBeforeArrived: unit is stuck; retrying move without roads` | §9 TIC | **Open.** 846 in 7 minutes, correlated with the ANTIFREEZE cluster. Diagnostic added 2026-08-29 (name + per-unit count); behaviour unchanged pending a re-fly. See `retlab-tic-dynamic-fronts-notes.md`. |
