...
1. **The pin moved** -- pydcs branch `dcs-2.9.29-surgical` (`bfdbb4d`), cut from the
   ...
   as the baseline: it ran with RetLab's OVGME F-4E pack applied, so for anything
...
2. **The extension sweep found four mismatches; two were drift and two were
   ...
   `tests/retlab/test_super_hornet_datalink.py` locks this, and CI proved it
...
The DCS update overwrote RetLab's `Expanded_F-4E_Weapons_Pack` OVGME mod. The live
...
[retlab-recon-role-scoping-notes.md](retlab-recon-role-scoping-notes.md).** That note
...
[`retlab-marianas-wwii-terrain-notes.md`](retlab-marianas-wwii-terrain-notes.md) "How the airfields
