...
> strategy ([`retlab-upstreaming-inventory.md`](../retlab-upstreaming-inventory.md)).
...
| `scar` init | 10 | **RetLab** | partial | Already uses MOOSE too; port the MIST helper calls to MOOSE utils. |
| `intercept-config` | 10 | RetLab/retribution glue | partial | Already uses MOOSE too; same as SCAR. |
...
`tars`, `tic`, `scar`, `c130j` (RetLab features); plus `MooseAtis`,
...
`splashdamage3` (RetLab PINNED locked build) has **zero** MIST dependency — the only matches are
...
2. **EWRS → DELETE** — ✅ **DONE (2026-06-24)**: redundant with the fork's MOOSE `bigeye`; removed
   ...
   See [`retlab-ewrs-retirement-decision.md`](retlab-ewrs-retirement-decision.md).
...
4. **CTLD → `Ops.CTLD`** — the large one (~8.7k lines, default-ON, ~26 distinct MIST calls). Config-
   ...
   pass. Full findings in [`retlab-ctld-mantis-style-port-scope.md`](retlab-ctld-mantis-style-port-scope.md).
5. **dismounts** — ✅ **RETIRED (2026-06-24)**. Was the would-be MIST-drop blocker (no MOOSE
   ...
   outright. See [`retlab-dismounts-decision.md`](retlab-dismounts-decision.md). No longer blocks the
   MIST drop.
...
[`retlab-moose-ops-opportunity-map.md`](retlab-moose-ops-opportunity-map.md).
...
- Coordinate each phase with [`retlab-upstreaming-inventory.md`](../retlab-upstreaming-inventory.md) — these
  are strong upstream-PR candidates (MANTIS, `Ops.CTLD`, MIST retirement).
- RetLab-authored pieces (`scar`, `intercept`, `c130j`, TIC/TARS) are already MOOSE and
  need no upstream coordination.
- RetLab PINNED Splash Damage build is unaffected (MIST-free).
...
marked in the file with a `-- RetLab patch (<subject>)` comment — grep for `RetLab patch`
...
`-- RetLab patch (event 61 spam)`. The 49/50 rows stay: they are upstream-shaped and
