...
1. **Do nothing** — GPS jamming is not implementable. (What every previous look
   ...
   `retlab-iads-c2-consequences-notes.md`, which records GPS jamming as *not
   feasible*.)
...
`GPS_GUIDED_WEAPON_PATTERNS` in `game/retlab/gps_jamming.py`, emitted to Lua
...
`tests/retlab/test_gps_jamming.py` pins both directions.
