# RetLab — IADS C2 family: felt consequences for command-center & power nodes (design notes)
...
`game/retlab/c2_decapitation.py` (linear with dead-CC fraction, `FULL_OFFENSIVE_PACKAGE_CAP`
...
> Skynet, plus the fork's dead stand-ins (`retlab-skynet-return-notes.md` §5).
