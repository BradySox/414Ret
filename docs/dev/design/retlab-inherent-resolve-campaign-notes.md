...
**Status: LANDED 2026-07-04 (headless-verified; needs an in-game pass).** RetLab's
...
`parse_phases` parse the authored blocks. CI-locked in `tests/retlab/test_inherent_resolve.py`.
...
- **Supply-route tracing** — the red-red ring/ratline waypoints are straight-ish first cuts;
  ...
  RetLab supply-line standard), as flagged in the yaml.
