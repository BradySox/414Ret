# RetLab — DCS 2.9.28 Iraq map content: what it unlocks — design notes
...
Related: [`retlab-desert-storm-campaign-notes.md`](retlab-desert-storm-campaign-notes.md),
[`retlab-inherent-resolve-campaign-notes.md`](retlab-inherent-resolve-campaign-notes.md),
[`retlab-scenery-import-notes.md`](retlab-scenery-import-notes.md) (the automation this plan
deliberately does **not** wait for), [`retlab-tanker-war-campaign-notes.md`](retlab-tanker-war-campaign-notes.md).
...
`tests/retlab/test_scenery_targets.py` guard runs the same check over every shipped campaign
...
3. **Parking/slot churn.** 2.9.28 touched Iraq airfield traffic; DS91 carries a standing
   parking-fit invariant (`tests/retlab/test_desert_storm.py`). Re-run it against the bumped
...
5. Register in `retlab-features.md` + `README.md` only once targets actually ship in a `.miz`.
