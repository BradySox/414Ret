# RetLab loadout/task/date integrity audit + fixes
...
`6b16d8d66` + `5647b67aa`, per `retlab-aircraft-task-rebalance-rubric.md`) that changes *which
...
Never showed at home because RetLab's own campaigns fly the F-14 as TARPS-only; the
...
New guard: `tests/retlab/test_navy_bomb_variants.py` — a fork-wide sweep asserting no
...
Guards added to `tests/retlab/test_navy_bomb_variants.py`: on a Navy jet the key may
