...
**Parent:** [`retlab-moose-ops-opportunity-map.md`](retlab-moose-ops-opportunity-map.md) (the Tier A/B/C
...
Organized by RetLab's feature pillars, with the planner/Lua verdict for each.

### Player experience & tasking — *the biggest opportunity*
RetLab is unusually **player-flown-feature heavy** (SCAR, TARPS recon, C-130 EW/ISR, JAMMING),
...
RetLab's identity is its **player-flown depth**. Today that's N bespoke menus. A deliberate
...
investment — a unified MOOSE player-task layer that would amplify RetLab's core identity.** The
