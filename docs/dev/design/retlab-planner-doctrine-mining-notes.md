...
- A checklist row if it has runtime behaviour CI cannot exercise, per
  [retlab-ingame-pass-checklist.md](../retlab-ingame-pass-checklist.md). Planner timing
...
[retlab-red-brain-phase0-notes.md](retlab-red-brain-phase0-notes.md) first. A list of things
...
**CAS pushes behind its SEAD window** — [RetLab#973](https://github.com/BradySox/RetLab/pull/973).
...
| 2 | **A TOT past the end of the mission window** | ✅ Built — [#975](https://github.com/BradySox/RetLab/pull/975), **on a justification that did not hold up.** The code defect is real and readable: the spread bounded the random offset by the cycle and then added transit on top, so nothing bounded the arrival. The share of packages it actually affected was first measured on hand-edited saves and was badly overstated; on fresh campaigns it is small. The fix stays because it is structurally right and is an identity for a zero-transit package, not because of a number. |
...
| 8 | **AI DEAD in net order, never at a dark site** | ✅ **Built 2026-09-15 in its minimal shape, on the DM's call ("seems really complex" to the full rule):** `DegradeIads` now offers the detector tier before the opportunistic LORAD/MERAD tier, so the EWRs covering a planned target are fragged before the SAMs they cover; the reactive tier is untouched (`tests/test_dead_net_order.py`). Flown pass owed: B126. **Checked the same evening on the DM's CSAR save and found inert there**: all three blue DEAD frags come from the reactive tier (24 SAMs threatening), and each red site is covered by 2–6 EWRs at 162–216 nm plus 1–4 SAM-as-EWR radars, so an EWR-first rule of either shape stacks rather than cuts the net; the one cuttable node is the command-centre pair, after which every site goes autonomous and live. Details in B126. The full net-order rule below stays recorded, unbuilt. **The finding:** Skynet lights a netted site only once the target is inside its kill zone (`GO_LIVE_WHEN_IN_KILL_ZONE` is the default, `skynet-iads-compiled.lua` line 3040), and DCS AI fires a HARM only at an emitter, so an AI DEAD flight at a netted site never gets a shot off: KIWI DEAD lost four F-16CM and its Mirage escort for zero shots; WEKA DEAD, same fit, against a site that had already lit up on its SEAD escort put eight HARMs on it and killed two launchers. A site goes autonomous, and Skynet then keeps it live from T0, when it has no covering parent (an EWR or SAM-as-EWR whose detection range reaches it, with power and comms), no usable command centre, or no active connection node (`setToCorrectAutonomousState`). **The rule:** `DegradeIads` frags an AI DEAD at a radar SAM only when the site is autonomous by that test. A netted site gets its net first: the detectors covering it, then its §52 command, comms and power nodes; the site itself comes the turn after, or the same turn behind them through §69's window. The reactive tier keeps its DEAD when nothing in the net is reachable, and a player-fragged DEAD is untouched. Needs one new query on `IadsNetwork`, covering parents by detection range, which is Skynet's own `buildRadarCoverage` test and is computable today from `max_detection_range`. Not the escort-as-bait shape (WEKA paid two Hornets to light its site) and not `adjustGoLiveRange` (global, gives up the sites-dark half for players too). Second-order: a launchers-only remnant such as OKAPI is outside Skynet and always live, so it is HARM-able, but `has_live_radar_sam` reads it as radar-less and drops the SEAD escort. |
