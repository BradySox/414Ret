# RetLab PR #823 Frontline Merge Notes
...
Pairs with `retlab-tic-dynamic-fronts-notes.md` (the TIC movement design) and CLAUDE.md §9
...
- **TIC build (RetLab default):** TANK/IFV/APC/ATGM hit the short-circuit (unchanged).
...
- **Docs to sync (doc-hygiene order):** `retlab-tic-dynamic-fronts-notes.md` (clustered laydown
  now feeds TIC), `docs/dev/retlab-features.md` §9 + §6, `README.md` (player-visible:
  default-stance setting + mixed clusters), `docs/dev/retlab-upstreaming-inventory.md` (record
...
- **Residual risk — in-game pass:** the merged laydown changes how front units spawn — a new
  ...
  `docs/dev/retlab-ingame-pass-checklist.md`, or fold it into the TIC in-game pass.
...
- **Buckets B + C — DONE (2026-06-26).** All in `game/missiongenerator/flotgenerator.py`:
  ...
  - **Tests:** the two held `tests/missiongenerator/test_flotgenerator_{movement,placement}.py`
    (#823, verbatim) + a new `test_flotgenerator_tic_guard.py` (RetLab) that locks the guard:
