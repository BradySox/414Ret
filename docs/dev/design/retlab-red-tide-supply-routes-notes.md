...
[`retlab-red-tide-campaign-notes.md`](retlab-red-tide-campaign-notes.md).
...
**User directive, now RetLab standard for ALL `supply_routes:` / shipping-lane
...
### Roll-out to the built campaigns (2026-07-03)

`tools/supply_route_geo.py` is now **multi-campaign** (`coin`, `red_flag_81_2`, and the
`caucasus_trail_fixes` partial). An audit converted every route endpoint in every campaign
back to real lat/lon and checked each corridor against the real road network:

- **Red Flag 81-2 (Nevada) — DONE.** All 8 supply routes (the FRONT polyline is left as the
  static-front geometry, not a road) re-traced through the tool onto the real NTTR roads:
  **US-95** (Nellis → Indian Springs → Camp Mercury; the western dogleg TTR → Goldfield →
  Scotty's Junction → Beatty), **US-6** east out of Tonopah, and the Gold Flat / Kawich
  interior. The genuine defect was the "US-95 west corridor" that ran straight down the
  Kawich Range instead of following US-95 west then south — now fixed. Reproduce with
  `python tools/supply_route_geo.py red_flag_81_2`.
- **Red Tide (Germany) — LEFT AS-IS.** Already densely traced along the real autobahns (the
  original supply-route work, above). Re-running the tool would be churn with regression
  risk; no change.
- **1968 Yankee Station + Steel Tiger (shared miz) — PARTIAL.** The three unambiguous
  defects are fixed in place (endpoints verbatim, so binding is unchanged; both yamls carry
  the identical block): **R3** Senaki → Thanh Hoa was a bare 2-point straight line (now
  descends the Kolkheti coastal plain); **R4/R5** the Ban Laboy trail legs overshot ~25 km
  east into the Senaki lowland and back (now run straight down the western foothill /
  descend cleanly to Senaki). Reproduce with `python tools/supply_route_geo.py
  caucasus_trail_fixes`. The **front (R0, Sochi → Gudauta coast road)** and the **Rioni
  route (R2, Senaki → Kutaisi)** were already good.
- **Khe Sanh (Caucasus) — CLEAN.** Its 4 routes are proper Rioni/mountain-valley descents
  and short coastal hops; no "wouldn't drive" defects, no change.

**Deferred to an in-app by-eye pass (Yankee Station / Steel Tiger, R6–R13).** These inland
trail-FOB corridors sit at fictional Samegrelo-highland placements, so per the standard they
want tracing on the DCS map, not headless lat/lon guessing. Endpoints are safe (binding is
by first/last waypoint), only the corridor shape needs a nudge. Priority order (worst first):
- **R6** Ban Karai → Ban Laboy, **R7** Ban Dong → Ban Karai, **R8** Ban Karai → Mu Gia,
  **R10** Ban Raving → Ky Son: the classic *clustered stub + one long straight jump* shape —
  the long leg likely cuts a ridge; replace with a 3–4 point valley descent between the FOBs.
- **R9** Ky Son → Ban Dong, **R11/R12** Ky Son ↔ Sukhumi, **R13** Tchepone → Sukhumi: milder
  — they already step through intermediate legs; only the leading stub cluster is noise.
- **R14** Tchepone → Gudauta and **R1** Da Nang → Khe Sanh are already well-traced; leave.
