...
assessment (`retlab-mist-author-repos-notes.md` §4, owed-work item 3). Every claim about
...
**In-house — §81 naval magazines** (`game/retlab/naval_magazines.py`,
...
- Four-test pattern to mirror: `tests/retlab/test_naval_magazines.py`,
...
1. `game/retlab/sam_magazines.py` — ledger, seeding, capacity, reconcile (≈ §81's 285
   lines).
...
8. Registry entry in `game/retlab/features.py` (CI-enforced), settings page, docs faces,
   in-game-pass rows.
