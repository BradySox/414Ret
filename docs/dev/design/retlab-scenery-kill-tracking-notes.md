...
> names in `DeadC2` is registered as a dead stand-in (`retlab-skynet-return-notes.md` §5).
...
`OBJECT ID` presence) and is CI-guarded by `tests/retlab/test_scenery_targets.py`. The
