# RetLab Single-Player Campaign Loop — "SP Pilot Mode"

> **STATUS: LANDED 2026-08-03 as feature §83** (S1 + S2 + S3; S4's guardrails are
> honoured throughout). Engineering detail lives in `retlab-features.md` §83; this note
...
> button, `game/retlab/sp_pilot_mode.py` (the aircraft-first board, ladder rungs 1-2)
> and `game/retlab/pre_turn_briefing.py` (the five-section pre-turn card, including
...
Meanwhile RetLab's multiplayer campaigns run to completion, on the same engine, with
...
...)`, and §44 (`game/retlab/carrier_ops.py`) builds `ProposedFlight`s with it and
