...
1. `docs/dev/design/retlab-campaign-architecture-notes.md` — the direction: five pillars,
   four admission rules, rungs R0–R7. **No tombstone is lifted by it**; each rung lands on
   its own DM call.
2. `docs/dev/design/retlab-substrate-inventory-notes.md` — R0, done. The verdict that
   reshaped R1: the core is already coherent; two private ledgers (§81/§63 magazines); five
   missing couplings.
3. `docs/dev/design/retlab-falcon-bms-campaign-notes.md` — the BMS study. Candidates 1 and 2
   are the scoped versions of rungs R2 and R7.
4. `docs/dev/design/retlab-region-priorities-notes.md` — §93, BUILT; the template for every
   command lever (weight, never fence).
5. `docs/dev/design/retlab-sam-magazines-notes.md` — parked; becomes R3 *on the substrate*,
...
1. **B65–B67 fly pass** (~1 evening; gates rung R2). Germany Red Tide is the natural
   campaign. Criteria per row in `docs/dev/retlab-ingame-pass-checklist.md`: reinforcement
...
3. ~~The detection-range check~~ **DONE 2026-08-20 — no defect found.** The two columns are not
   ...
   *is* a DCS-box task if it is ever wanted. Written up in `retlab-mist-author-repos-notes.md` §3.
...
3. **R1 proper on the DM's call** — one persistence home for magazine stocks: absorb
   `game/retlab/naval_magazines.py` (§81) and the §63 cruise state channel, shaped so
