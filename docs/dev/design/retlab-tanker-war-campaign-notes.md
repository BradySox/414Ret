...
`tests/retlab/test_tanker_war.py`, README + CLAUDE.md design-notes entries, checklist row
...
Goal: a period-correct naval campaign that *runs* and is fun, before any RetLab identity layer.
...
Deliverable: flyable, period-correct, no RetLab layer yet.

### Phase 2 — RetLab naval-war identity
...
3. Register the campaign (feature index / any preseeds), add a `tests/retlab/`-style guard
...
Rapier + AAA, oil platforms ×60 + Silkworm ×35, **0 empty AA sites**. `pytest tests/retlab
...
RetLab naval-war identity layer, all in `tanker_war_1988.yaml` (no code, no miz edit):
...
AA sites); `pytest tests/retlab tests/armedforces tests/test_newgame_settings.py` → 396 passed.
...
tuning. Docs registration + the `tests/retlab/test_tanker_war.py` laydown guard landed
