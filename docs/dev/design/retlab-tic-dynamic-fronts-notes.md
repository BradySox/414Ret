# RetLab TIC Dynamic Fronts Notes
...
RetLab drives TIC entirely through waypoint NAMES (`t+N hdg=H roe=simulate`) — no DCS
...
- `simulate` ROE and the stationary-only fire behavior are intentional. Don't switch to
  `roe=kill` (judged too lethal/accurate by RetLab).
