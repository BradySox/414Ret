# RetLab — "Vietnam Retribution" mode — HANDOFF (start here)

**Status:** design landed, **no implementation yet**.
**Read first:** [`retlab-vietnam-retribution-notes.md`](retlab-vietnam-retribution-notes.md) — the
...
- **Adding a tasking-behavior change (P3+)** → add a row to
  `docs/dev/retlab-ingame-pass-checklist.md` (CI can't exercise runtime AI behavior).
...
Per CLAUDE.md docs-hygiene: design note → `docs/dev/retlab-features.md` (new §) → `README.md`
(player-facing) → register in `game/retlab/features.py` → checklist row → CLAUDE.md feature
list → sync `AGENTS.md`.
