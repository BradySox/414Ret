...
| File | What / why |
|---|---|
| `resources/campaigns/red_tide.yaml` | The campaign. Differences from the fork's copy: **(1)** the fork-only YAML `supply_routes:`/`shipping_lanes:` blocks are removed (upstream reads routes from miz groups — see the miz row); **(2)** the four squadron-identity squadron names are swapped for squadron defs that already exist upstream (`RetLab Voodoo Squadron`→`23rd FS`, `RetLab Hornets`→`VMFA-251`, `414th Tactical Fighter Squadron`→`336th Fighter Squadron`, `RetLab Aviation Detachment`→`HMLA-269 (UH-1H)`), and the one description line naming RetLab is neutralized; **(3)** `aircraft_type: C-130J-30` (a fork/mod consolidation) → vanilla `C-130`; **(4)** `recommended_enemy_faction: Russia 1980` → `Russia 1988` (below). |
