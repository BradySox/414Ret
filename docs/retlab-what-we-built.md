# RetLab — What RetLab Built on Top of DCS Retribution

*A short tour of the work behind the squadron's fork of [DCS Retribution](https://github.com/dcs-retribution/dcs-retribution).*

DCS Retribution is the open-source dynamic-campaign generator we fly. **RetLab** is the
a fork of it — the same proven campaign engine, with a thick
...
*(Full per-feature engineering detail lives in [`docs/dev/retlab-features.md`](dev/retlab-features.md).)*
...
[roadmap](dev/retlab-community-contribution-roadmap.md) for handing the rest back.
...
*RetLab is maintained by the RetLab. Source:
<https://github.com/BradySox/RetLab>. Built on the open-source
