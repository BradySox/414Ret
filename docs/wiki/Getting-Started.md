...
> 414Ret keeps the upstream Retribution engine and adds a squadron-focused layer on top: incomplete enemy intelligence that makes recon worth flying, new player tasks (SCAR, Combat SAR, C-130J JAMMING, TARPS), reworked air-defense and front-line behavior, a unified dark-themed map layers panel, a Vietnam campaign layer, and several built campaigns (*Germany — Red Tide*, *1968 Yankee Station*, *Red Flag 81-2*, and the Afghanistan COIN campaign *Operation Enduring Resolve*). Where this guide notes a difference, it links to the relevant page. For the full tour, see [414th Fork Overview](414th-Fork-Overview).
