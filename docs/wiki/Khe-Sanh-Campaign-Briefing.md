...
![A C-130 on the Khe Sanh airstrip amid supply pallets, smoke, and the surrounding hills, 1968](https://raw.githubusercontent.com/bradyccox/414Ret/main/docs/campaigns/img/khe-sanh/khe-sanh-c130-takeoff.jpg)
<sub>The lifeline under fire — a C-130 on the Khe Sanh strip, 1968. *USAF photo (public domain), via Wikimedia Commons; full [image credits](Khe-Sanh-Visual-Briefing#image-credits--sources).*</sub>
...
> F-100, F-4E, B-52, C-130J**. **See [Khe Sanh — Required Mods](Khe-Sanh-Required-Mods)** for the
...
| **The Vietnam layer** | **Political will + Regime Resolve** meters, the **negotiation ending**, the **static front**, and the authored **Rolling Thunder → Linebacker II ROE arc** — see §3 and the [Vietnam Campaign Layer](Vietnam-Campaign-Layer) page |
| **Vietnam Ops suite** | All ON except naval gunfire (inland siege): Arc Light, flak gauntlet, **convoy interdiction (Route 9)**, **airbase harassment** (the strip takes rocket/mortar fire), **Super Gaggle**, FAC(A) willie pete, snake-and-nape — see [Vietnam Ops](Vietnam-Ops) |
...
*The generic mechanics are documented on the **[Vietnam Campaign Layer](Vietnam-Campaign-Layer)**
...
made playable: the LBJ situation-room photo in the [intel pack](Khe-Sanh-Intel-Assessment) is
...
- **CAS / FAC — the main effort.** A-1H Skyraider (the iconic "Sandy", Kutaisi + Da Nang), A-4E
  ...
  (the faction's JTAC airframe — and the [willie-pete marking](Vietnam-Ops#7--faca-willie-pete-target-marking)
...
- **Recon — TARPS, not armed recon.** The RF-101B and RA-5C are **unarmed photo birds** flying
  [TARPS](TARPS-Reconnaissance): their film **confirms BDA and lifts the recon fog** (what you
...
| **P-37 "Bar Lock" EWR** | Early-warning radar | **One site, the NVA deep rear** — the eyes feeding the SA-2 net and the MiG GCI (HDS ships ON; see [Required Mods](Khe-Sanh-Required-Mods)). Kill it and the net goes blind. |

> The **AAA belt is ~30 emplacement groups** across the siege ring and the approaches, plus the
> [flak gauntlet](Vietnam-Ops#2--aaa-flak-gauntlet) runtime: predictable, steady flying draws
> tightening barrage bursts; jinking loosens them. The **airstrip takes real harassment fire** too —
> the [airbase harassment](Vietnam-Ops#5--airbase-harassment-rocketmortar-siege) runtime drops
...
> The [FAC(A) marking runtime](Vietnam-Ops#7--faca-willie-pete-target-marking) drops real white
...
**campaign mechanic, not just mood**. Full system docs: [Combat SAR](Combat-SAR).
...
> [snake-and-nape](Vietnam-Ops#8--snake-and-nape-napalm-cas) low-pass runtime — press the run in
...
the campaign-layer mechanics are documented on [Vietnam Campaign Layer](Vietnam-Campaign-Layer) and
the period runtime suite on [Vietnam Ops](Vietnam-Ops). The ROE arc and phase ribbon are
