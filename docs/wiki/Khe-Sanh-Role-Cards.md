...
[mission brief](Khe-Sanh-Campaign-Briefing#7--the-mission-brief-template).*

> 🟢🟡 **History & loadouts.** Tactics and ordnance are period-correct for 1968 — **dumb iron,
> napalm, rockets, and guns; no PGMs, no SAMs to dodge, no MANPADS.** The campaign does **not**
> date-restrict weapons by default (`restrict_weapons_by_date` off), so the loadout editor may offer
> later stores — fly the era kit for accuracy. Several airframes are **modern DCS stand-ins** for the
> period type (noted on each card).
...
[Visual Briefing](Khe-Sanh-Visual-Briefing). Loadouts are recommended period baselines; actual
