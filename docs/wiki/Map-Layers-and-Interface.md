...
- **Target Intel panel.** Every ground-object dialog opens with a read-only intel block:
  ...
  ![The target intel dialog for an unscouted enemy SAM site, with known units, detection range, and threat range all reading "Unknown (not scouted)"](https://raw.githubusercontent.com/BradySox/414Ret/main/docs/wiki/img/fog-intel-not-scouted.png)
