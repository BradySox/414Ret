...
ending (checked first in ``check_win_loss``) outranks both. Evaluation is
...
def victory_verdict(game: Game) -> Optional[Literal["win", "loss"]]:
    """The §75 alternate ending, or None while the war goes on.

    Backs the branch in ``Game.check_win_loss`` between the W2 negotiation
    ending (which outranks this) and the stock territory checks. Loss precedence
    within the module -- the W2 rule: a simultaneous collapse is never a cheap
    win.
    """
