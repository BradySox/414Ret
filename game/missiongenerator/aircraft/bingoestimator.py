...
class BingoEstimator:
    ...
    def _fuel_consumption_based_estimate(self, fuel: FuelConsumption) -> int:
        # Bingo is the cruise fuel to the recovery field from the most distant
        # point on the route, plus the landing reserve. A REFUEL waypoint is not a
        # fuel source here: a planned tanker pass is not gas taken, and bingo is
        # the number the pilot turns home on if the tanker is not there.
        farthest_nm = self._max_distance_from(self.arrival).nautical_miles
