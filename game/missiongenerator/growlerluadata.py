...
The ESCORT_JAMMER planner role puts a dedicated jammer -- the EA-18G Growler or
EA-6B Prowler, the only airframes that declare the ``Escort Jammer`` task (§77)
-- on the package's join->split leg; this emits each such flight and the package
group names it protects, and the ``growler`` plugin drives the scripted jamming
effects at runtime (the Timberwolf/Matador escort-geometry family the C-130 §2
...
group by name + geometry) jams automatically. No ESCORT_JAMMER flight -> no node
-> the plugin no-ops.
"""
