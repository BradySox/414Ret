...
class LuaGenerator:
    ...
    def _has_scramble_flights(self) -> bool:
        """True if the mission contains at least one RED SCRAMBLE flight."""
        return any(
            not f.friendly.is_blue and f.flight_type == FlightType.SCRAMBLE
    ...
    def _inject_scramble_script(self) -> None:
        ...
        This fires only when RED SCRAMBLE flights are present, making reactive
        GCI interception a first-class Retribution feature rather than an
        optional plugin.  Zones are already auto-generated by generate_plugin_data(),
        so no manual zone placement in the ME is needed.
        """
        if not self._has_scramble_flights():
            return
        script_path = Path("./resources/plugins/scramble/reactive_scramble.lua")
        if not script_path.exists():
            logging.error(
                "reactive_scramble.lua not found at %s — SCRAMBLE flights will "
                "orbit on WeaponHold indefinitely",
                script_path.resolve(),
            )
            return
    ...
    def generate_plugin_data(self) -> None:
        ...
        # Auto-generate a DCS trigger zone for every RED SCRAMBLE flight's home
        # airbase.  reactive_scramble.lua picks these up via the SCRAMBLE_<name>
        # fallback — no manual zone placement in the ME is needed.
        #
        # We also emit dcsRetribution.barcap_zones so the lua finds them via the
        # fast table lookup first, avoiding a getZone() call per scan tick.
        # Zone radius: ~100 nm — comfortably wider than the default engage radius
        # (~51 nm) so interceptors have time to reach the threat.
            ...
            if not flight.friendly.is_blue and flight.flight_type == FlightType.SCRAMBLE:
                ...
                if target_name and target_name not in barcap_zones and hasattr(target, "position"):
                    ...
                    self.mission.triggers.add_triggerzone(
                        target.position,
                        radius=_SCRAMBLE_ZONE_RADIUS,
                        hidden=True,
                        name=zone_name,
                    )
                    barcap_zones[target_name] = zone_name

        if barcap_zones:
            lines = ["dcsRetribution.barcap_zones = {}"]
            for airbase, zone in barcap_zones.items():
                lines.append(
