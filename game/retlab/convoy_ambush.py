...
* each ambush team is a real, hidden red TGO placed by
  :func:`game.retlab.coin.spawn_red_ground_at` -- killing it is a real red ground loss
...
and the ambient-convoy layer -- ``game/retlab/ambient_convoys.py``, which keeps a few
...
def seed_convoy_ambushes(game: "Game", events: Any) -> None:
    ...
    from game.retlab.coin import CELL_SIDC, ambush_unit_types, spawn_red_ground_at
...
def _despawn_prior_ambushes(game: "Game", state: dict[str, Any], events: Any) -> None:
    ...
    from game.retlab.coin import _despawn, _tgo_by_id
