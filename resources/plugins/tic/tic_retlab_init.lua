-- RetLab TIC initialization & ambient-fire extension.
...
if GLSCO == nil or GLSCO_COMBATANT == nil or GLSCO_BATTLEFIELD == nil then
    env.warning("tic_retlab_init: TIC is not loaded; skipping RetLab TIC setup")
    return
end
...
if ambientFireEnabled() then
    env.info("tic_retlab_init: ambient fire extension enabled")
    ...
    env.info("tic_retlab_init: ambient fire extension disabled by plugin option")
end
...
local init_ok, init_err = pcall(function()
    env.info("tic_retlab_init: initializing TIC battle")
    GLSCO:Initialize()
    if GLSCO.battle ~= nil then
        env.info("tic_retlab_init: activating TIC battle")
        GLSCO.battle:Activate()
    else
        env.warning("tic_retlab_init: GLSCO.battle is nil after Initialize!")
    end
end)
if not init_ok then
    env.warning("tic_retlab_init: TIC battle init failed: " .. tostring(init_err))
end
