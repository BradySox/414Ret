...
# Generic AAA sites must be optically-guided guns. Two guards, because either
# alone can be undone: the slot only accepts `AAARadar` (so a SAM search radar
# cannot match it at all) and it does not auto-fill (so a faction's own Fire Can
# is not handed to guns that cannot use it). A preset that bundles the radar with
# its guns still places one.
...
def test_aaa_radar_slot_is_not_auto_filled(layout_name: str) -> None:
    ...
    radar_groups = [
        ug for ug in layout.all_unit_groups if UnitClass.AAA_RADAR in ug.unit_classes
    ]
...
def test_fire_can_preset_still_keeps_its_radar() -> None:
    ...
    assert any(unit.unit_class is UnitClass.AAA_RADAR for unit in group.units)
