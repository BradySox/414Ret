...
``14:28Z 17:28L`` puts both clocks in one cell. The letters distinguish them on a
careful read; the colour does it at a glance. Zulu leads and keeps the page's own
foreground, because it is the figure that matches the DED.
...
from game.missiongenerator.kneeboard import LOCAL_CELL_TOKEN, KneeboardPageWriter

HEADERS = ["#", "Time"]
ROWS = [["0", "14:28Z 17:28L"], ["1", "14:33Z 17:33L"]]
...
def test_the_token_is_the_cell_form_not_the_prose_form() -> None:
    # The BLUF's "14:53:16Z (17:53:16L)" is parenthesised and reads apart already;
    # matching inside it would colour the "53:16L" tail of one number.
    assert LOCAL_CELL_TOKEN.findall("14:28Z 17:28L") == ["17:28L"]
    assert LOCAL_CELL_TOKEN.findall("14:53:16Z (17:53:16L)") == []


def test_a_highlighted_table_puts_the_local_figure_in_the_nav_colour() -> None:
    writer = KneeboardPageWriter(dark_theme=False)
    writer.table(
        ROWS, headers=HEADERS, highlight=LOCAL_CELL_TOKEN, highlight_fill=writer.col_nav
    )
