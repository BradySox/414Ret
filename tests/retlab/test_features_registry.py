"""RetLab feature registry must stay honest.
...
from game.retlab.features import (
    FEATURE_INDEX_DOC,
    FEATURES,
...
def test_features_doc_pointers_resolve() -> None:
    ...
    assert not missing, (
        ...
        f"docs/dev/retlab-features.md: {missing}"
    )


# Features whose engineering detail deliberately lives under a differently-numbered
# heading in retlab-features.md, rather than one of their own. Keep this small: a new
...
def test_every_listed_feature_has_a_features_doc_section() -> None:
    ...
    assert not missing, (
        ...
        f"docs/dev/retlab-features.md: {missing} — write the section, or add a "
