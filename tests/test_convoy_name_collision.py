...
Ported from upstream dcs-retribution#928; the generator test is adapted because the
fork's generate_convoy() resolves a spawn plan before it builds the group.
"""
...
from game.missiongenerator.convoygenerator import ConvoyGenerator, SpawnPlan
...
def test_a_save_that_already_has_duplicates_still_generates() -> None:
    ...
    Drives the real generate_convoy() with the spawn plan and group construction
    stubbed out -- what is under test is the name it asks for, not the pydcs group.
    """
    ...
    generator.spawn_plan = lambda _convoy: SpawnPlan(position, None, False)  # type: ignore[arg-type]
