...
def test_role_callsign_is_registered_into_the_country_pool() -> None:
    # "Toxic" isn't a stock DCS callsign -> the spawner injects it so pydcs can
    # resolve it instead of raising ValueError.
    pool = ["Enfield", "Springfield"]
    _spawner("Toxic", "Plane", pool)._register_custom_callsign()
    assert "Toxic" in pool


def test_role_callsign_injection_is_idempotent() -> None:
    pool = ["Enfield"]
    spawner = _spawner("Toxic", "Plane", pool)
    spawner._register_custom_callsign()
    spawner._register_custom_callsign()
    assert pool.count("Toxic") == 1
...
def test_role_callsign_is_deregistered_after_spawn() -> None:
    ...
    # the pool) hands Toxic to unrelated auto-named flights. This is the
    # "callsign applied to all aircraft" regression.
    pool = ["Enfield", "Springfield"]
    spawner = _spawner("Toxic", "Plane", pool)
    spawner._register_custom_callsign()
    assert "Toxic" in pool  # present only while this group is being spawned
...
def test_deregister_is_idempotent_and_skips_stock_callsigns() -> None:
    ...
    _spawner("Toxic", "Plane", pool)._deregister_custom_callsign()
    assert pool == ["Enfield"]
