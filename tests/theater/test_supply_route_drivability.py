...
`caucasus_iron_gate`'s Kutaisi/Khashuri route reached the Likhi range in one 41 km
...
def iron_gate() -> Any:
    ...
    campaign = Campaign.from_file(Path("resources/campaigns/caucasus_iron_gate.yaml"))
...
def test_the_yaml_override_replaced_the_miz_path(iron_gate: Any) -> None:
    """22 authored waypoints, not the 8 of the miz group it overrides."""
    route = _route(iron_gate, "Kutaisi", "Khashuri FOB")

    assert len(route) == 22


def test_the_override_did_not_link_the_pair_twice(iron_gate: Any) -> None:
    ...
    """
    for cp in iron_gate.controlpoints:
        names = [other.name for other in cp.connected_points]
        assert len(names) == len(set(names)), f"{cp.name} is linked twice: {names}"


def test_every_waypoint_is_on_ground_a_convoy_can_drive(iron_gate: Any) -> None:
    zone = iron_gate.landmap.inclusion_zone_only
    route = _route(iron_gate, "Kutaisi", "Khashuri FOB")
...
def test_the_drawn_line_follows_drivable_ground(iron_gate: Any) -> None:
